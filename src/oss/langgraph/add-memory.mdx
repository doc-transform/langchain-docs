...
AI 应用需要[记忆](/oss/concepts/memory)来在多次交互中共享上下文。在 LangGraph 中，你可以添加两种类型的记忆：

* [添加短期记忆](#add-short-term-memory)：将其作为 agent [状态](/oss/langgraph/graph-api#state)的一部分，以支持多轮对话。
* [添加长期记忆](#add-long-term-memory)：跨会话存储用户级别或应用级别的数据。

## 添加短期记忆 {#add-short-term-memory}

**短期**记忆（线程级[持久化](/oss/langgraph/persistence)）使 agent 能够跟踪多轮对话。添加短期记忆的方式如下：
...
<Accordion title="示例：使用 Postgres checkpointer">
  ...
  ```
    ...
    <Tip>
    首次使用 Postgres checkpointer 时，需要调用 `checkpointer.setup()`
    </Tip>
...
<Accordion title="示例：使用 MongoDB checkpointer">
  ...
  ```
    ...
    <Note>
    **设置**
    要使用 [MongoDB checkpointer](https://pypi.org/project/langgraph-checkpoint-mongodb/)，你需要一个 MongoDB 集群。如果你还没有，请按照[此指南](https://www.mongodb.com/docs/guides/atlas/cluster/)创建一个集群。
    </Note>
...
<Accordion title="示例：使用 Redis checkpointer">
  ...
  ```
    ...
    <Tip>
    首次使用 Redis checkpointer 时，需要调用 `checkpointer.setup()`。
    </Tip>
...
如果你的图包含[子图](/oss/langgraph/use-subgraphs)，只需在编译父图时提供 checkpointer 即可。LangGraph 会自动将 checkpointer 传播到子图中。
...
你可以配置子图特定的检查点行为。查看[子图持久化](/oss/langgraph/use-subgraphs#subgraph-persistence)以了解持久化级别的详情，包括中断支持和有状态的续传。
...
一旦你使用 store 编译了图，LangGraph 会自动将 store 注入到节点函数中。推荐通过 `Runtime` 对象访问 store。
...
<Accordion title="示例：使用 Postgres store">
  ...
  ```
    ...
    <Tip>
    首次使用 Postgres store 时，需要调用 `store.setup()`
    </Tip>
...
<Accordion title="示例：使用 Redis store">
  ...
  ```
    ...
    <Tip>
    首次使用 [Redis store](https://pypi.org/project/langgraph-checkpoint-redis/) 时，需要调用 `store.setup()`。
    </Tip>
...
在图的记忆 store 中启用语义搜索，让 agent 可以通过语义相似度搜索 store 中的条目。
...
启用[短期记忆](#add-short-term-memory)后，较长的对话可能会超出 LLM 的上下文窗口。常见的解决方案有：
...
这使得 agent 能够在不超出 LLM 上下文窗口的前提下持续跟踪对话。

### 裁剪消息 {#trim-messages}

:::python
大多数 LLM 都有最大支持的上下文窗口（以 token 为单位）。一种决定何时截断消息的方法是计算消息历史中的 token 数，并在接近限制时进行截断。如果你使用的是 LangChain，可以使用 trim messages 工具，指定要从列表中保留的 token 数量，以及处理边界的 `strategy`（例如保留最后 `max_tokens` 个 token）。
:::
:::js
大多数 LLM 都有最大支持的上下文窗口（以 token 为单位）。一种决定何时截断消息的方法是计算消息历史中的 token 数，并在接近限制时进行截断。如果你使用的是 LangChain，可以使用 trim messages 工具，指定要从列表中保留的 token 数量，以及处理边界的 `strategy`（例如保留最后 `maxTokens` 个 token）。
:::

:::python
要裁剪消息历史，使用 [`trim_messages`](https://python.langchain.com/api_reference/core/messages/langchain_core.messages.utils.trim_messages.html) 函数：
...
你可以从图状态中删除消息来管理消息历史。当你想要移除特定消息或清空整个消息历史时，这很有用。

:::python
要从图状态中删除消息，可以使用 `RemoveMessage`。要使 `RemoveMessage` 生效，你需要使用带有 @[`add_messages`] [reducer](/oss/langgraph/graph-api#reducers) 的状态键，例如 [`MessagesState`](/oss/langgraph/graph-api#messagesstate)。
...
:::js
要从图状态中删除消息，可以使用 `RemoveMessage`。要使 `RemoveMessage` 生效，你需要使用带有 @[`messagesStateReducer`][messagesStateReducer] [reducer](/oss/langgraph/graph-api#reducers) 的状态键，例如 `MessagesValue`。
...
<Warning>
删除消息时，**请确保**删除后的消息历史是有效的。请检查你所使用的 LLM 提供商的限制。例如：
...
如上所示，裁剪或删除消息的问题在于你可能会在清理消息队列时丢失信息。因此，某些应用可以从更精密的方法中受益，即使用聊天模型对消息历史进行摘要。

![Summary](/oss/images/summary.png)


:::python
可以使用提示词和编排逻辑来对消息历史进行摘要。例如，在 LangGraph 中，你可以扩展 [`MessagesState`](/oss/langgraph/graph-api#working-with-messages-in-graph-state) 来包含一个 `summary` 键：
...
然后，你可以生成聊天历史的摘要，并将现有摘要作为下一次摘要的上下文。这个 `summarize_conversation` 节点可以在 `messages` 状态键中累积了一定数量的消息后被调用。
...
:::js
可以使用提示词和编排逻辑来对消息历史进行摘要。例如，在 LangGraph 中，你可以在状态中在 `messages` 键旁边包含一个 `summary` 键：
...
然后，你可以生成聊天历史的摘要，并将现有摘要作为下一次摘要的上下文。这个 `summarizeConversation` 节点可以在 `messages` 状态键中累积了一定数量的消息后被调用。
...
```
  ...
  1. 我们将在 `context` 字段中跟踪我们的滚动摘要

  （这是 `SummarizationNode` 所需的）。

  1. 定义仅用于过滤

  `call_model` 节点输入的私有状态。

  1. 我们在这里传递私有输入状态，以隔离摘要节点返回的消息
  ...
  const workflow = new StateGraph(GraphState)
    ...
    // 这意味着这些是 `conversation` 节点调用后采取的边。
      "conversation",
    // 接下来，我们传入决定下一个节点的函数。
      shouldContinue,
    )
    // 现在添加一条从 `summarize_conversation` 到 END 的普通边。
    // 这意味着 `summarize_conversation` 调用后，我们就结束了。
...
如果你使用任何数据库支持的持久化实现（如 Postgres 或 Redis）来存储短期和/或长期记忆，你需要在使用数据库之前运行迁移来设置所需的 schema。

按照惯例，大多数数据库特定的库会在 checkpointer 或 store 实例上定义一个 `setup()` 方法来运行所需的迁移。但你应该查看你的 @[`BaseCheckpointSaver`] 或 @[`BaseStore`] 的具体实现来确认精确的方法名和用法。

我们建议将迁移作为专门的部署步骤运行，或确保它们作为服务器启动的一部分运行。
