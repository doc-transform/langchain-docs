...
LangGraph 实现了一套流式系统来实时呈现更新。流式输出对于提升基于 LLM 构建的应用的响应性至关重要。通过渐进式显示输出——即使在完整响应准备好之前——流式输出显著改善了用户体验（UX），尤其是在处理 LLM 延迟时。
...
:::python
LangGraph 图暴露了 @[`stream`][Pregel.stream]（同步）和 @[`astream`][Pregel.astream]（异步）方法，以迭代器形式产出流式输出。传入一个或多个[流模式](#stream-modes)来控制你接收的数据。
...
:::js
LangGraph 图暴露了 @[`stream`][Pregel.stream] 方法，以迭代器形式产出流式输出。
...
向 `stream()` 或 `astream()` 传入 `version="v2"` 以获得统一的输出格式。每个 chunk 是一个具有一致结构的 `StreamPart` 字典——无论流模式、模式数量或子图设置如何：
...
每种流模式都有对应的 `TypedDict`，包括 @[`ValuesStreamPart`]、@[`UpdatesStreamPart`]、@[`MessagesStreamPart`]、@[`CustomStreamPart`]、@[`CheckpointStreamPart`]、@[`TasksStreamPart`]、@[`DebugStreamPart`]。你可以从 `langgraph.types` 导入这些类型。联合类型 @[`StreamPart`] 是基于 `part["type"]` 的不相交联合，支持编辑器和类型检查器中的完整类型窄化。

使用 v1（默认）时，输出格式会根据流选项变化（单模式返回原始数据，多模式返回 `(mode, data)` 元组，子图返回 `(namespace, data)` 元组）。使用 v2 时，格式始终相同：
...
v2 格式还支持类型窄化，这意味着你可以通过 `chunk["type"]` 过滤 chunk 并获得正确的载荷类型。每个分支将 `part["data"]` 窄化为该模式的特定类型：
...
| 模式 | 类型 | 描述 |
| :--- | :--- | :---------- |
| [values](#graph-state) | @[`ValuesStreamPart`] | 每步后的完整状态。 |
| [updates](#graph-state) | @[`UpdatesStreamPart`] | 每步后的状态更新。同一步中的多个更新会分别流式输出。 |
| [messages](#llm-tokens) | @[`MessagesStreamPart`] | 来自 LLM 调用的 (LLM token, 元数据) 二元组。 |
...
| [tasks](#tasks) | @[`TasksStreamPart`] | 任务启动/完成事件，包含结果和错误。需要检查点器。 |
...
| [updates](#graph-state) | 每步后的状态更新。同一步中的多个更新会分别流式输出。 |
| [messages](#llm-tokens) | 来自 LLM 调用的 (LLM token, 元数据) 二元组。 |
...
使用流模式 `updates` 和 `values` 来流式输出图执行时的状态。

* `updates` 流式输出图每步后状态的**更新部分**。
* `values` 流式输出图每步后状态的**完整值**。
...
<Tabs>
    <Tab title="updates">
    使用此模式来仅流式输出节点在每步后返回的**状态更新**。流式输出包含节点名称以及更新内容。
...
:::python
`messages` 模式的流式输出是一个 `(message_chunk, metadata)` 元组，其中：

* `message_chunk`：来自 LLM 的 token 或消息片段。
* `metadata`：包含图节点和 LLM 调用详细信息的字典。

> 如果你的 LLM 没有 LangChain 集成，你可以使用 `custom` 模式来流式输出它的结果。详见[与任意 LLM 配合使用](#use-with-any-llm)。

<Warning>
**Python < 3.11 异步需要手动配置**
当使用 Python < 3.11 的异步代码时，你必须显式地将 @[`RunnableConfig`] 传递给 `ainvoke()` 以启用正确的流式输出。详见 [Python < 3.11 的异步](#async) 或升级到 Python 3.11+。
</Warning>
...
:::python
要从 LangGraph 节点或工具内部发送**自定义用户定义数据**，请按以下步骤操作：

1. 使用 @[`get_stream_writer`] 访问流写入器并发出自定义数据。
2. 调用 `.stream()` 或 `.astream()` 时设置 `stream_mode="custom"` 以在流中获取自定义数据。你可以组合多个模式（例如 `["updates", "custom"]`），但至少有一个必须是 `"custom"`。

<Warning>
**Python < 3.11 异步不支持 @[`get_stream_writer`]**
在 Python < 3.11 的异步代码中，@[`get_stream_writer`] 无法工作。
应改为在节点或工具中添加 `writer` 参数并手动传递。
详见 [Python < 3.11 的异步](#async) 使用示例。
</Warning>
...
2. 调用 `.stream()` 时设置 `streamMode: "custom"` 以在流中获取自定义数据。你可以组合多个模式（例如 `["updates", "custom"]`），但至少有一个必须是 `"custom"`。
...
要发出 `on_tool_event` 事件，将工具函数定义为**异步生成器**（`async function*`）。每次 `yield` 将中间数据发送到流，`return` 值作为工具的最终结果。
```typescript
import { tool } from "@langchain/core/tools";
import { z } from "zod/v4";
...
`@langchain/langgraph-sdk/react` 中的 `useStream` 钩子在你的流模式中包含 `"tools"` 时会暴露一个 `toolProgress` 数组。每个条目是一个 `ToolProgress` 对象，跟踪正在运行的工具的当前状态：
...
| `input` | 工具的输入参数 |
| `data` | 来自 `on_tool_event` 的最新 yield 数据 |
...
<Accordion title="扩展示例：带工具进度的旅行规划 Agent">

本示例展示了一个完整的 Agent，包含异步生成器工具，可将搜索进度流式输出到 React UI。
...
#### `tools` 与 `custom` 流模式的区别

两种流模式都可以呈现工具进度，但它们服务于不同目的：

* **`tools`**——自动发出结构化的生命周期事件（`on_tool_start`、`on_tool_event`、`on_tool_end`、`on_tool_error`），无需在工具中做任何代码修改，只需使用 `async function*`。`useStream` 钩子开箱即用提供响应式的 `toolProgress` 数组。
* **`custom`**——让你完全控制何时以及通过 `config.writer()` 发出什么数据。当你需要不与工具生命周期对应的自由格式数据，或者想从节点（而不仅仅是工具）流式输出时使用。
...
:::python
要在流式输出中包含来自[子图](/oss/langgraph/use-subgraphs)的输出，你可以在父图的 `.stream()` 方法中设置 `subgraphs=True`。这将同时流式输出父图和所有子图的输出。

输出以 `(namespace, data)` 元组的形式流式输出，其中 `namespace` 是到子图被调用节点的路径元组，例如 `("parent_node:<task_id>", "child_node:<task_id>")`。
...
<Accordion title="扩展示例：从子图流式输出">
  ...
  **注意**，我们不仅接收到节点更新，还接收到命名空间，它告诉我们从哪个图（或子图）进行流式输出。
</Accordion>

:::python
### 检查点

使用 `checkpoints` 流模式在图执行时接收检查点事件。每个检查点事件的格式与 `get_state()` 的输出相同。需要[检查点器](/oss/langgraph/persistence)。
...
使用 `tasks` 流模式在图执行时接收任务启动和完成事件。任务事件包含关于哪个节点正在运行、其结果和任何错误的信息。需要[检查点器](/oss/langgraph/persistence)。
...
使用 `debug` 流模式在图执行过程中流式输出尽可能多的信息。流式输出包含节点名称以及完整状态。
...
:::python
<Note>
`debug` 模式结合了 `checkpoints` 和 `tasks` 事件以及额外的元数据。如果只需要调试信息的一个子集，直接使用 `checkpoints` 或 `tasks`。
</Note>
:::
...
:::python
你可以将列表作为 `stream_mode` 参数传入，同时流式输出多个模式。

使用 `version="v2"` 时，每个 chunk 都是一个 `StreamPart` 字典。使用 `chunk["type"]` 来区分不同模式：
...
:::js
你可以将数组作为 `streamMode` 参数传入，同时流式输出多个模式。

流式输出将是 `[mode, chunk]` 元组，其中 `mode` 是流模式名称，`chunk` 是该模式流式输出的数据。
...
:::python
你可以使用 `stream_mode="custom"` 从**任意 LLM API** 流式输出数据——即使该 API **未**实现 LangChain 聊天模型接口。

这让你可以集成原始 LLM 客户端或提供自己流式接口的外部服务，使 LangGraph 在自定义配置中非常灵活。
...
:::js
你可以使用 `streamMode: "custom"` 从**任意 LLM API** 流式输出数据——即使该 API **未**实现 LangChain 聊天模型接口。
...
如果你的应用混合使用支持流式输出和不支持流式输出的模型，你可能需要显式地为不支持流式输出的模型禁用流式输出。
...
v2 流式格式（本页通篇使用）提供了统一的输出格式。以下是主要差异和迁移方式的摘要：

| 场景 | v1（默认） | v2（`version="v2"`） |
|---|---|---|
| 单流模式 | 原始数据（字典） | 包含 `type`、`ns`、`data` 的 `StreamPart` 字典 |
| 多流模式 | `(mode, data)` 元组 | 相同的 `StreamPart` 字典，通过 `chunk["type"]` 过滤 |
| 子图流式输出 | `(namespace, data)` 元组 | 相同的 `StreamPart` 字典，检查 `chunk["ns"]` |
| 多模式 + 子图 | `(namespace, mode, data)` 三元组 | 相同的 `StreamPart` 字典 |
| `invoke()` 返回类型 | 普通字典（状态） | 包含 `.value` 和 `.interrupts` 的 `GraphOutput` |
| 中断位置（流） | 状态字典中的 `__interrupt__` 键 | `values` 流部分的 `interrupts` 字段 |
| 中断位置（调用） | 结果字典中的 `__interrupt__` 键 | `GraphOutput` 的 `.interrupts` 属性 |
| Pydantic/dataclass 输出 | 返回普通字典 | 强制转换为模型/dataclass 实例 |

#### v2 invoke 格式

当你向 `invoke()` 或 `ainvoke()` 传入 `version="v2"` 时，它返回一个 @[`GraphOutput`] 对象，包含 `.value` 和 `.interrupts` 属性：
...
当使用 `"values"` 以外的流模式时，`invoke(..., stream_mode="updates", version="v2")` 返回 `list[StreamPart]` 而不是 `list[tuple]`。

<Warning>
在 `GraphOutput` 上使用字典式访问（`result["key"]`、`"key" in result`、`result["__interrupt__"]`）仍然可用以保持向后兼容，但已**弃用**，将在未来版本中移除。请迁移到 `result.value` 和 `result.interrupts`。
</Warning>

这将状态与中断元数据分离。使用 v1 时，中断信息嵌入在返回字典的 `__interrupt__` 下：
...
1. 你**必须**显式地将 [`RunnableConfig`](https://python.langchain.com/docs/concepts/runnables/#runnableconfig) 传递给异步 LLM 调用（如 `ainvoke()`），因为回调不会自动传播。
2. 你**不能**在异步节点或工具中使用 @[`get_stream_writer`]——必须直接传递 `writer` 参数。

<Accordion title="扩展示例：异步 LLM 调用手动传递 config">
  ```python
  from typing import TypedDict
  from langgraph.graph import START, StateGraph
  from langchain.chat_models import init_chat_model
...
<Accordion title="扩展示例：异步自定义流式输出与流写入器">
  ```python
  from typing import TypedDict
