...
在本教程中，我们将使用 LangGraph 构建一个[检索](/oss/langchain/retrieval) Agent。

LangChain 提供了内置的 [Agent](/oss/langchain/agents) 实现，基于 [LangGraph](/oss/langgraph/overview) 原语实现。如果需要更深入的自定义，可以直接在 LangGraph 中实现 Agent。本指南演示了检索 Agent 的示例实现。[检索](/oss/langchain/retrieval) Agent 在你希望 LLM 决定是从向量存储中检索上下文还是直接响应用户时非常有用。

在本教程结束时，我们将完成以下内容：

1. 获取和预处理用于检索的文档。
2. 将这些文档索引用于语义搜索，并为 Agent 创建检索器工具。
...
- 使用[文档加载器](/oss/integrations/document_loaders)、[文本分割器](/oss/integrations/splitters)、[嵌入向量](/oss/integrations/text_embedding)和[向量存储](/oss/integrations/vectorstores)进行[检索](/oss/langchain/retrieval)
...
<Tip>
  注册 LangSmith 以快速发现问题并提升 LangGraph 项目的性能。[LangSmith](https://docs.smith.langchain.com) 让你使用追踪数据来调试、测试和监控基于 LangGraph 构建的 LLM 应用。
</Tip>

## 1. 预处理文档

:::python
1. 获取用于 RAG 系统的文档。我们将使用 [Lilian Weng 的优秀博客](https://lilianweng.github.io/) 中最新的三篇文章。首先使用 `WebBaseLoader` 工具获取页面内容：
  ```python
...
:::js
1. 获取用于 RAG 系统的文档。我们将使用 [Lilian Weng 的优秀博客](https://lilianweng.github.io/) 中最新的三篇文章。首先使用 `CheerioWebBaseLoader` 获取页面内容：
  ```typescript
...
1. 使用内存向量存储和 OpenAI 嵌入向量：
  ...
  ```
2. 使用 LangChain 预建的 `createRetrieverTool` 创建检索器工具：
  ```typescript
...
现在我们开始构建 Agentic RAG 图的组件（[节点](/oss/langgraph/graph-api#nodes)和[边](/oss/langgraph/graph-api#edges)）。

:::python
注意，这些组件将在 [`MessagesState`](/oss/langgraph/graph-api#messagesstate) 上运行——这是包含 `messages` 键的图状态，其中存储了 [chat 消息](https://python.langchain.com/docs/concepts/messages/)列表。

1. 构建 `generate_query_or_respond` 节点。它将调用 LLM 根据当前图状态（消息列表）生成响应。根据输入消息，它将决定使用检索器工具进行检索，还是直接响应用户。注意，我们通过 `.bind_tools` 让聊天模型访问之前创建的 `retriever_tool`：
  ```python
...
:::js
1. 构建 `generateQueryOrRespond` 节点。它将调用 LLM 根据当前图状态（消息列表）生成响应。根据输入消息，它将决定使用检索器工具进行检索，还是直接响应用户。注意，我们通过 `.bindTools` 让聊天模型访问之前创建的 `tools`：
  ```typescript
...
:::python
1. 添加一个[条件边](/oss/langgraph/graph-api#conditional-edges)——`grade_documents`——来判断检索到的文档是否与问题相关。我们将使用带有结构化输出 schema `GradeDocuments` 的模型进行文档评分。`grade_documents` 函数将根据评分结果返回要跳转的节点名称（`generate_answer` 或 `rewrite_question`）：
  ```python
...
:::js
1. 添加一个节点——`gradeDocuments`——来判断检索到的文档是否与问题相关。我们将使用带有 Zod 结构化输出的模型进行文档评分。我们还会添加一个[条件边](/oss/langgraph/graph-api#conditional-edges)——`checkRelevance`——检查评分结果并返回要跳转的节点名称（`generate` 或 `rewrite`）：
  ```typescript
...
:::python
1. 构建 `generate_answer` 节点：如果通过了评分检查，我们可以根据原始问题和检索到的上下文生成最终答案：
  ```python
  GENERATE_PROMPT = (
...
* 使用 `tools_condition` 路由到下一步：
  ...
  * 否则，直接响应用户
* 评分检索到的文档内容与问题的相关性（`grade_documents`），然后路由到下一步：
  * 如果不相关，使用 `rewrite_question` 重写问题，然后再次调用 `generate_query_or_respond`
  * 如果相关，继续到 `generate_answer`，使用包含检索到的文档上下文的 @[`ToolMessage`] 生成最终响应
...
* 使用条件边路由到下一步：
  ...
  * 否则，直接响应用户
* 评分检索到的文档内容与问题的相关性（`gradeDocuments`），然后路由到下一步：
  * 如果不相关，使用 `rewrite` 重写问题，然后再次调用 `generateQueryOrRespond`
  * 如果相关，继续到 `generate`，使用包含检索到的文档上下文的 @[`ToolMessage`] 生成最终响应
