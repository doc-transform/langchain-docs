...
LangGraph 提供了两种不同的 API 来构建 Agent 工作流：**Graph API** 和 **Functional API**。两种 API 共享相同的底层运行时，可以在同一应用中组合使用，但它们是针对不同的用例和开发偏好设计的。

本指南将帮助你根据具体需求理解何时使用哪种 API。
...
[Graph API](/oss/langgraph/graph-api) 使用声明式方法，你定义节点、边和共享状态来创建可视化的图结构。

**1. 复杂的决策树和分支逻辑**

当你的工作流有多个依赖于各种条件的决策点时，Graph API 使这些分支显式且易于可视化。
...
当你需要在工作流的不同部分之间共享和协调状态时，Graph API 的显式状态管理很有优势。
...
当你需要并行运行多个操作然后合并结果时，Graph API 可以自然地处理这种情况。
...
[Functional API](/oss/langgraph/functional-api) 使用命令式方法，将 LangGraph 功能集成到标准的过程式代码中。
...
当你希望快速测试想法，而不需要定义状态 schema 和图结构的开销时。
...
你可以在同一应用中同时使用两种 API。当系统的不同部分有不同的需求时，这很有用。
...
当你的 Functional 工作流变得复杂时，你可以迁移到 Graph API：
...
当你需要对工作流结构的显式控制、复杂分支、并行处理或团队协作优势时，选择 **Graph API**。

当你希望以最小的更改为现有代码添加 LangGraph 功能、拥有简单的线性工作流或需要快速原型开发能力时，选择 **Functional API**。

两种 API 都提供相同的核心 LangGraph 功能（持久化、流式输出、人工干预、记忆），但以不同的范式包装它们，以适应不同的开发风格和用例。
