---
title: "代码分割 - 文本分割器集成指南"
---

@[RecursiveCharacterTextSplitter] 包含预置的分隔符列表，用于按特定编程语言[分割文本](/oss/integrations/splitters/)。
...
要查看给定语言的分隔符列表，请将此枚举的值传入
```python
RecursiveCharacterTextSplitter.get_separators_for_language
```

要实例化一个针对特定语言的分割器，请将枚举值传入
```python
