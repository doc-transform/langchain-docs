...
description: "使用 LangChain JavaScript 集成 Mozilla/readability 文档转换器。"
---

在接入 HTML 文档以便后续检索时，我们通常只关心网页的实际内容而非语义标记。
使用 MozillaReadabilityTransformer 剥离 HTML 标签可以得到内容更丰富的文本块，从而使检索更加有效。
...
```
虽然转换器本身不需要，但下面的用法示例需要使用 [`cheerio`](https://www.npmjs.com/package/cheerio) 进行网页抓取：
...
下面的示例抓取了一个 Hacker News 帖子，根据 HTML 标签进行分割以根据标签的语义信息对文本块进行分组，
然后从各个文本块中提取内容：
...
你可以向转换器传递 [`@mozilla/readability` 包接受的任何参数](https://www.npmjs.com/package/@mozilla/readability)来自定义其工作方式。
