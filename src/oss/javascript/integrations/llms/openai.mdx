---
title: "OpenAI 集成"
description: "使用 LangChain JavaScript 集成 OpenAI LLM 模型。"
---

<Warning>
**你当前正在查看 OpenAI 文本补全模型的使用文档。最新且最流行的 OpenAI 模型是[对话补全模型](/oss/langchain/models)。**

除非你正在专门使用 `gpt-3.5-turbo-instruct`，否则你可能需要的是[这个页面](/oss/integrations/chat/openai/)。
</Warning>

[OpenAI](https://en.wikipedia.org/wiki/OpenAI) 是一家人工智能（AI）研究实验室。

本文将帮助你开始使用 OpenAI 文本补全模型（LLM）。OpenAI 所有功能和配置选项的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_openai.OpenAI.html)。
...
要访问 OpenAI 模型，你需要创建 OpenAI 账户、获取 API 密钥，并安装 `@langchain/openai` 集成包。
...
你还可以传入官方 SDK 支持的其他 `ClientOptions` 参数。

如果你在 Azure OpenAI 上托管，请参见[专门页面](/oss/integrations/llms/azure)。

---

## API 参考

所有 OpenAI 功能和配置的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_openai.OpenAI.html)。
