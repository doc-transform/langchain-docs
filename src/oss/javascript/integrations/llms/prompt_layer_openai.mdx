...
description: "使用 LangChain JavaScript 集成 PromptLayer OpenAI LLM 模型。"
---
...
LangChain 与 PromptLayer 集成，用于日志记录和调试提示词和响应。要添加 PromptLayer 支持：

1. 在此处创建 PromptLayer 账户：[https://promptlayer.com](https://www.promptlayer.com/)。
2. 创建 API 令牌，并将其作为 `PromptLayerOpenAI` 构造函数的 `promptLayerApiKey` 参数传入，或设置为 `PROMPTLAYER_API_KEY` 环境变量。
...
LangChain 还与 PromptLayer 集成以支持 Azure 托管的 OpenAI 实例：
