---
title: "AzureOpenAIEmbeddings 嵌入集成"
sidebarTitle: "嵌入"
description: "使用 LangChain JavaScript 集成 AzureOpenAIEmbeddings 嵌入模型。"
---

[Azure OpenAI](https://azure.microsoft.com/products/ai-services/openai-service/) 是一项云服务，可帮助你使用 OpenAI、Meta 等公司提供的多种预构建和精选模型快速开发生成式 AI 体验。
...
你可以在[这个页面](https://learn.microsoft.com/azure/ai-services/openai/overview)了解更多关于 Azure OpenAI 及其与 OpenAI API 的差异。如果你还没有 Azure 账户，可以[创建一个免费账户](https://azure.microsoft.com/free/)来开始使用。

本文将帮助你开始使用 AzureOpenAIEmbeddings [嵌入模型](/oss/integrations/text_embedding)。`AzureOpenAIEmbeddings` 所有功能和配置选项的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_openai.AzureOpenAIEmbeddings.html)。


<Info>
**以前，LangChain.js 支持使用专用的 [Azure OpenAI SDK](https://github.com/Azure/azure-sdk-for-js/tree/main/sdk/openai/openai) 与 Azure OpenAI 集成。该 SDK 现已弃用，建议使用 OpenAI SDK 中的新 Azure 集成，它允许在发布当天即可访问最新的 OpenAI 模型和功能，并允许在 OpenAI API 和 Azure OpenAI 之间无缝切换。**


如果你正在使用已弃用 SDK 的 Azure OpenAI，请参见[迁移指南](#migration-from-azure-openai-sdk)来更新到新 API。
...
要访问 Azure OpenAI 嵌入模型，你需要创建 Azure 账户、获取 API 密钥，并安装 `@langchain/openai` 集成包。

### 凭据

你需要部署一个 Azure OpenAI 实例。你可以按照[此指南](https://learn.microsoft.com/azure/ai-services/openai/how-to/create-resource?pivots=web-portal)在 Azure Portal 上部署一个版本。

当你的实例运行后，确保你拥有实例名称和密钥。你可以在 Azure Portal 的实例“密钥和端点”部分找到密钥。
...
<Tip>
**如果未定义 `AZURE_OPENAI_API_EMBEDDINGS_DEPLOYMENT_NAME`，它将回退使用 `AZURE_OPENAI_API_DEPLOYMENT_NAME` 的值作为部署名称。同样，`AzureOpenAIEmbeddings` 构造函数中的 `azureOpenAIApiEmbeddingsDeploymentName` 参数也会在未定义时回退使用 `azureOpenAIApiDeploymentName` 的值。**
...
嵌入模型通常用于检索增强生成（RAG）流程中，既用于数据索引，也用于后续检索。更详细的说明请参见[学习](/oss/learn/)页面下的 RAG 教程。

以下展示了如何使用上面初始化的 `embeddings` 对象来索引和检索数据。在此示例中，我们将使用演示版 [`MemoryVectorStore`](/oss/integrations/vectorstores/memory) 来索引和检索示例文档。
...
在底层，向量存储和检索器实现分别调用 `embeddings.embedDocument(...)` 和 `embeddings.embedQuery(...)` 来为 `fromDocuments` 中使用的文本和检索器的 `invoke` 操作创建嵌入。

你可以直接调用这些方法来获取你自己用例的嵌入。

### 嵌入单个文本

你可以使用 `embedQuery` 嵌入搜索查询。这会生成特定于查询的向量表示：
...
你可以使用 `embedDocuments` 嵌入多个文本用于索引。此方法的内部实现可能（但不一定）与嵌入查询不同：
...
如果你的实例托管在默认 `openai.azure.com` 之外的域名下，你需要使用备用的 `AZURE_OPENAI_BASE_PATH` 环境变量。
...
如果你正在使用已弃用的 `@langchain/azure-openai` 包的 Azure OpenAI SDK，你可以按照以下步骤更新代码以使用新的 Azure 集成：
...
2. 更新导入语句，使用 `@langchain/openai` 包中的新 `AzureOpenAIEmbeddings` 类：

   ```typescript
   import { AzureOpenAIEmbeddings } from "@langchain/openai";
   ```

3. 更新代码以使用新的 `AzureOpenAIEmbeddings` 类并传递所需参数：
...
所有 AzureOpenAIEmbeddings 功能和配置的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_openai.AzureOpenAIEmbeddings.html)。
