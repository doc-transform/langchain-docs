---
title: "TogetherAIEmbeddings 嵌入集成"
description: "使用 LangChain JavaScript 集成 TogetherAIEmbeddings 嵌入模型。"
---

本文将帮助你开始使用 TogetherAIEmbeddings [嵌入模型](/oss/integrations/text_embedding)。`TogetherAIEmbeddings` 所有功能和配置选项的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_community_embeddings_togetherai.TogetherAIEmbeddings.html)。
...
要访问 TogetherAI 嵌入模型，你需要创建 TogetherAI 账户、获取 API 密钥，并安装 `@langchain/community` 集成包。

### 凭据

你可以在[这里](https://api.together.xyz/)注册 Together 账户并创建 API 密钥。完成后设置 `TOGETHER_AI_API_KEY` 环境变量：
...
所有 TogetherAIEmbeddings 功能和配置的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_community_embeddings_togetherai.TogetherAIEmbeddings.html)。
