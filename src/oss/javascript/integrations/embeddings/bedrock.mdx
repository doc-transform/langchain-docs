---
title: "BedrockEmbeddings 嵌入集成"
sidebarTitle: "嵌入"
description: "使用 LangChain JavaScript 集成 BedrockEmbeddings 嵌入模型。"
---


[Amazon Bedrock](https://aws.amazon.com/bedrock/) 是一项完全托管的服务，通过单一 API 提供来自 AI21 Labs、Anthropic、Cohere、Meta、Stability AI 和 Amazon 等领先 AI 公司的高性能基础模型（FM），以及构建具有安全性、隐私性和负责任 AI 的生成式 AI 应用程序所需的广泛能力。

本文将帮助你开始使用 Amazon Bedrock [嵌入模型](/oss/integrations/text_embedding)。`Bedrock` 所有功能和配置选项的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_aws.BedrockEmbeddings.html)。
...
要访问 Bedrock 嵌入模型，你需要创建 AWS 账户、获取 API 密钥，并安装 `@langchain/aws` 集成包。

前往 [AWS 文档](https://docs.aws.amazon.com/bedrock/latest/userguide/getting-started.html)注册 AWS 并设置你的凭据。你还需要为你的账户开启模型访问权限，可以按照[这些说明](https://docs.aws.amazon.com/bedrock/latest/userguide/model-access.html)操作。
...
有几种不同的 AWS 身份验证方式 — 以下示例依赖于在环境变量中设置的访问密钥、秘密访问密钥和区域：
...
所有 Bedrock 功能和配置的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_aws.BedrockEmbeddings.html)。
