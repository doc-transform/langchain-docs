---
title: "GoogleGenerativeAIEmbeddings 嵌入集成"
sidebarTitle: "嵌入"
description: "使用 LangChain JavaScript 集成 GoogleGenerativeAIEmbeddings 嵌入模型。"
---

本文将帮助你开始使用 Google Generative AI [嵌入模型](/oss/integrations/text_embedding)。`GoogleGenerativeAIEmbeddings` 所有功能和配置选项的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_google_genai.GoogleGenerativeAIEmbeddings.html)。
...
要访问 Google Generative AI 嵌入模型，你需要注册 Google AI 账户、获取 API 密钥，并安装 `@langchain/google-genai` 集成包。

### 凭据

在这里获取 API 密钥：[ai.google.dev/tutorials/setup](https://ai.google.dev/tutorials/setup)。

然后，将你的密钥设置为名为 `GOOGLE_API_KEY` 的环境变量：
...
LangChain `GoogleGenerativeAIEmbeddings` 集成位于 `@langchain/google-genai` 包中。你可能还希望安装官方 SDK：
...
所有 `GoogleGenerativeAIEmbeddings` 功能和配置的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_google_genai.GoogleGenerativeAIEmbeddings.html)。
