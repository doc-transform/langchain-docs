---
title: "CohereEmbeddings 嵌入集成"
description: "使用 LangChain JavaScript 集成 CohereEmbeddings 嵌入模型。"
---

本文将帮助你开始使用 CohereEmbeddings [嵌入模型](/oss/integrations/text_embedding)。`CohereEmbeddings` 所有功能和配置选项的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_cohere.CohereEmbeddings.html)。
...
要访问 Cohere 嵌入模型，你需要创建 Cohere 账户、获取 API 密钥，并安装 `@langchain/cohere` 集成包。
...
### Cohere 在 Azure、AWS Bedrock 和独立 Cohere 实例上的自定义客户端
...
所有 CohereEmbeddings 功能和配置的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_cohere.CohereEmbeddings.html)。
