---
title: "字节豆包嵌入集成"
description: "使用 LangChain JavaScript 集成字节豆包嵌入模型。"
---

本文将帮助你开始使用字节豆包 [嵌入模型](/oss/integrations/text_embedding)。`ByteDanceDoubaoEmbeddings` 所有功能和配置选项的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/_langchain_community.embeddings_bytedance_doubao.ByteDanceDoubaoEmbeddings.html)。
...
你需要注册 [ARK API 密钥](https://console.volcengine.com/ark/region:ark+cn-beijing/apiKey)并将其设置为名为 `ARK_API_KEY` 的环境变量。然后你需要为嵌入模型[创建一个接入点](https://console.volcengine.com/ark/region:ark+cn-beijing/endpoint)，并使用接入点的名称作为 `model`。
...
所有 ByteDanceDoubaoEmbeddings 功能和配置的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/_langchain_community.embeddings_bytedance_doubao.ByteDanceDoubaoEmbeddings.html)。
