---
title: "Composio 集成"
description: "通过 Composio 的统一 API 平台访问 500 多个工具和集成，支持 OAuth 处理、事件驱动工作流和多用户支持。"
---

[Composio](https://composio.dev) 是一个集成平台，提供对 GitHub、Slack、Notion 等 500 多个流行应用工具的访问。它使 AI 代理能够通过统一的 API 与外部服务交互，处理身份验证、权限和事件驱动工作流。
...
设置 [LangSmith](/langsmith/home) 进行追踪也很有用：
...
使用 LangChain 提供者初始化 Composio，并从特定工具包获取工具。每个工具包代表一个服务（如 GitHub、Slack），包含多个工具（你可以执行的操作）。
...
以下是一个完整的示例，使用 Composio 工具和 LangGraph 代理与 HackerNews 交互：
...
Composio 支持基于外部事件触发代理。当连接的应用中发生事件（如新的 GitHub 提交或 Slack 消息）时，触发器会自动将结构化负载发送到你的应用。
...
对于生产环境，在 [Composio 控制台](https://platform.composio.dev/settings/events)中配置 webhooks：
...
更多详情请参见 [Composio 触发器文档](https://docs.composio.dev/docs/using-triggers)

## 高级功能

### 自定义工具

Composio 允许你创建自定义工具，可以与内置工具一起使用。有两种类型：
...
更多详情请参见 [Composio 自定义工具文档](https://docs.composio.dev/docs/custom-tools)
