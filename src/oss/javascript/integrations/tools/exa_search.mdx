...
Exa（原名 Metaphor Search）是一个专为 LLM 使用而设计的搜索引擎。使用自然语言查询在互联网上搜索文档，然后从所需文档中检索清理后的 HTML 内容。

与基于关键词的搜索（Google）不同，Exa 的神经搜索功能使其能够从语义上理解查询并返回相关文档。例如，我们可以搜索“关于猫的有趣文章”并比较 Google 和 Exa 的搜索结果。Google 给我们的是基于“有趣”关键词的 SEO 优化列表文章。Exa 则直接返回相关结果。

本页介绍了如何将 `ExaSearchResults` 与 LangChain 一起使用。
...
首先，获取 Exa API 密钥并将其添加为环境变量。在[此处](https://dashboard.exa.ai/login)注册可获得每月 1000 次免费搜索。

```typescript
process.env.EXASEARCH_API_KEY="your-api-key"
```

设置 LangSmith 也很有用（但非必需），可以获得最佳的可观测性：
...
以下展示了如何实例化 `ExaSearchResults` 工具：
...
所有 `ExaSearchResults` 功能和配置的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_exa.ExaSearchResults.html)
