...
**AWS Step Functions** 是一种可视化工作流服务，帮助开发者使用 AWS 服务构建分布式应用程序、自动化流程、编排微服务以及创建数据和机器学习（ML）管道。

通过在提供给代理的工具列表中包含 `AWSSfn` 工具，你可以赋予代理调用 AWS 云中运行的异步工作流的能力。

当代理使用 `AWSSfn` 工具时，它会提供一个 `string` 类型的参数，该参数将被传入该工具支持的某个操作中。支持的操作有：`StartExecution`、`DescribeExecution` 和 `SendTaskSuccess`。
...
- 如果你还没有通过 AWS CLI 运行 [`aws configure`](https://docs.aws.amazon.com/cli/latest/userguide/cli-chap-configure.html)，则必须向 AWSSfn 构造函数提供 `region`、`accessKeyId` 和 `secretAccessKey`。
- 与这些凭据对应的 IAM 角色必须有权限调用 Step Function。
