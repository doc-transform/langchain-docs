...
本文将帮助你开始使用 [Watsonx 文档压缩器](/oss/concepts/#document_compressors)。所有 Watsonx 文档压缩器功能和配置的详细文档请参见 [API 参考](https://api.js.langchain.com/modules/_langchain_community.document_compressors_ibm.html)。
...
要访问 IBM WatsonxAI 模型，你需要创建一个 IBM watsonx.ai 账户，获取 API 密钥或其他类型的凭据，并安装 `@langchain/community` 集成包。
...
将这些变量设置到环境变量中并初始化对象后，身份验证将自动进行。
...
首先，使用嵌入、文本分割器和向量存储搞建一个基本的 RAG 接入流水线。我们将用它来对与选定查询相关的文档进行重排序：
...
或者，你也可以让文档随结果一起返回，为此可以使用如下所示的 `.compressDocuments()` 方法。
...
所有 Watsonx 文档压缩器功能和配置的详细文档请参见 [API 参考](https://api.js.langchain.com/modules/_langchain_community.document_compressors_ibm.html)。
