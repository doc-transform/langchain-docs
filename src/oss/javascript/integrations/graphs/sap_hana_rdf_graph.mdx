...
前提条件：
- 启用了 triple store 功能的 SAP HANA Cloud 实例
- 参见：[启用 Triple Store](https://help.sap.com/docs/hana-cloud-database/sap-hana-cloud-sap-hana-database-knowledge-graph-guide/enable-triple-store/)

要在 LangChain 中使用 SAP HANA 知识图谱引擎，请安装 `@sap/hana-langchain` 包及其依赖：
...
* **`graphUri`**：你的 RDF 数据所在的命名图（或 `"DEFAULT"`）
* **以下之一**：
  1. **`ontologyQuery`**：一个用于提取 schema 三元组的 SPARQL CONSTRUCT 查询
  2. **`ontologyUri`**：托管的本体图 URI
  3. **`ontologyLocalFile`** + **`ontologyLocalFileFormat`**：本地 Turtle/RDF 文件
  4. **`autoExtractOntology: true`**（不建议用于生产环境——参见下文注释）

`graphUri` 与本体的区别

* **`graphUri`**：
  你的 SAP HANA Cloud 实例中包含实例数据的命名图（有时超过 10 万个三元组）。
  如果未提供 `graphUri`、提供 `""` 或 `"DEFAULT"`，则使用默认图。
* **本体**：一个精简的 schema（通常约 50-100 个三元组），描述类、属性、域、值域、标签、注释和子类关系。本体指导 SPARQL 生成和结果解释。

### 使用 **DEFAULT** 图创建图实例

关于 DEFAULT 图的更多信息参见 [DEFAULT 图和命名图](https://help.sap.com/docs/hana-cloud-database/sap-hana-cloud-sap-hana-database-knowledge-graph-guide/default-graph-and-named-graphs)。
...
使用自定义的 `CONSTRUCT` 查询来选择性地提取 schema 三元组。
...
> **注意**：自动提取**不建议**用于生产环境——它通常会遏漏重要的三元组，如 `rdfs:label`、`rdfs:comment` 和 `rdfs:subClassOf`。
