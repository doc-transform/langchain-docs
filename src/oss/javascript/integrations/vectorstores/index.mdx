...
[向量存储](/oss/integrations/vectorstores) 存储[嵌入](/oss/integrations/text_embedding)数据并执行相似性搜索。
...
这种抽象让你可以在不同实现之间切换，而无需更改应用逻辑。

### 初始化

LangChain 中大多数向量存储在初始化时接受嵌入模型作为参数。
...
你可以使用 `addDocuments` 函数向向量存储添加文档。
...
使用 `similaritySearch` 发起语义查询，返回最接近的嵌入文档：
...
高效搜索通常采用 HNSW（分层可导航小世界）等索引方法，具体取决于向量存储的实现。

### 元数据过滤

按元数据（如来源、日期）过滤可以精细搜索结果：
...
LangChain.js 与各种向量存储集成。你可以查看下面的完整列表：
