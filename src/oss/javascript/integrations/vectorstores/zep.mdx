...
> 对 Zep Cloud 感兴趣？请参见 [Zep Cloud 安装指南](https://help.getzep.com/sdks)

**注意：** `ZepVectorStore` 与 `Documents` 一起工作，旨在用作 `Retriever`。
它提供了与 Zep 的 `ZepMemory` 类不同的功能，后者专为持久化、丰富和搜索用户聊天历史而设计。
...
你需要 Zep API URL，以及可选的 API 密钥来使用 Zep VectorStore。有关更多信息，请参见 [Zep 文档](https://help.getzep.com/welcome)。

在下面的示例中，我们使用 Zep 的自动嵌入功能，该功能使用低延迟嵌入模型在 Zep 服务器上自动嵌入文档。由于 LangChain 需要传入 @[`Embeddings`] 实例，我们传入 `FakeEmbeddings`。

**注意：** 如果你传入非 `FakeEmbeddings` 的 @[`Embeddings`] 实例，该类将用于嵌入文档。你还必须将文档集合设置为 `isAutoEmbedded === false`。请参见下方的 `OpenAIEmbeddings` 示例。
