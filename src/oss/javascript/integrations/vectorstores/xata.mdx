...
[Xata](https://xata.io) 是一个基于 PostgreSQL 的无服务器数据平台。它提供类型安全的 TypeScript/JavaScript SDK 用于与数据库交互，并提供用于管理数据的 UI。

Xata 具有原生向量类型，可以添加到任何表中，并支持相似性搜索。LangChain 直接将向量插入到 Xata，并查询给定向量的最近邻，因此你可以将所有 LangChain 嵌入集成与 Xata 一起使用。
...
在 [Xata UI](https://app.xata.io) 中创建一个新数据库。你可以随意命名，但在此示例中我们使用 `langchain`。
创建一个表，同样可以随意命名，但我们将使用 `vectors`。通过 UI 添加以下列：

- `content`，类型为 “Text”。这用于存储 `Document.pageContent` 值。
- `embedding`，类型为 “Vector”。使用你计划使用的模型的维度（OpenAI 为 1536）。
- 任何其他你想用作元数据的列。它们从 `Document.metadata` 对象中填充。例如，如果在 `Document.metadata` 对象中有一个 `title` 属性，你可以在表中创建一个 `title` 列，它将被自动填充。
...
然后选择你之前创建的数据库。这也会生成一个 `xata.ts` 或 `xata.js` 文件，定义了你可以用来与数据库交互的客户端。有关使用 Xata JavaScript/TypeScript SDK 的更多详情，请参见 [Xata 入门文档](https://xata.io/docs/getting-started/installation)。
...
本示例使用 `VectorDBQAChain` 搜索存储在 Xata 中的文档，然后将它们作为上下文传递给 OpenAI 模型，以回答用户提出的问题。
...
本示例演示了如何使用 LangChain.js 和 Xata 实现语义搜索。在运行之前，确保在 Xata 的 `vectors` 表中添加一个类型为 String 的 `author` 列。
