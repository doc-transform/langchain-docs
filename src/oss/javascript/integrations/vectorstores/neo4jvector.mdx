...
要使用 Neo4j Vector Index，你需要安装 `neo4j-driver` 包：
...
你可以在 [Neo4j 官网](https://neo4j.com/docs/operations-manual/current/installation/) 上找到更多关于设置 `Neo4j` 的信息。
...
_安全提示_：确保数据库连接使用权限范围严格限制的凭据，仅包含必要的权限。如果不这样做，可能导致数据损坏或丢失，因为调用代码可能在适当提示时尝试执行删除、修改数据的命令，或在数据库中存在敏感数据时读取这些数据。防止此类负面结果的最佳方法是（根据情况）限制授予此工具使用的凭据的权限。例如，为数据库创建只读用户是确保调用代码不能修改或删除数据的好方法。
