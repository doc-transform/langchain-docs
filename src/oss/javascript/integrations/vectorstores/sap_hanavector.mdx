...
>[SAP HANA Cloud Vector Engine](https://help.sap.com/docs/hana-cloud-database/sap-hana-cloud-sap-hana-database-vector-engine-guide/sap-hana-cloud-sap-hana-database-vector-engine-guide) 是一个完全集成到 `SAP HANA Cloud 数据库` 的向量存储。

## 设置

安装 `@sap/hana-langchain` 外部集成包，以及本教程中使用的其他包。
...
确保你的 SAP HANA 实例正在运行。从环境变量加载凭据并使用你首选的 HANA 客户端创建连接。
...
了解更多关于 SAP HANA 的信息，请参见[什么是 SAP HANA？](https://www.sap.com/products/data-cloud/hana/what-is-sap-hana.html)。

## 初始化

要初始化 `HanaDB` 向量存储，你需要一个数据库连接和一个嵌入实例。SAP HANA Cloud Vector Engine 支持外部和内部嵌入。
...
或者，你可以直接在 SAP HANA 中使用其原生 `VECTOR_EMBEDDING()` 函数计算嵌入。如果你的 TypeScript 环境中有内部嵌入支持，请初始化并将其传递给 `HanaDB`。有关内部嵌入的更多信息，请参见 [SAP HANA VECTOR_EMBEDDING 函数](https://help.sap.com/docs/hana-cloud-database/sap-hana-cloud-sap-hana-database-vector-engine-guide/vector-embedding-function-vector)。

> 注意：确保你的 SAP HANA Cloud 实例已启用 NLP。
...
拥有连接和嵌入实例后，通过将它们与用于存储向量的表名一起传递给 `HanaDB` 来创建向量存储：
...
你可以在初始化 `HanaDB` 实例时通过 `distanceStrategy` 参数指定距离策略。
...
有关在数据库级别创建索引的更多信息，请参考[官方文档](https://help.sap.com/docs/hana-cloud-database/sap-hana-cloud-sap-hana-database-vector-engine-guide/create-vector-index-statement-data-definition)。
...
除了基本的基于值的过滤能力外，还可以使用更高级的过滤。下表显示了可用的过滤操作符。
...
| `$between` | 在两个边界值的范围之间 |
| `$like` | 基于 SQL 中 "LIKE" 语义的文本等价（使用 "%" 作为通配符） |
| `$contains` | 过滤包含特定关键词的文档 |
| `$and` | 逻辑 "与"，支持两个或更多操作数 |
| `$or` | 逻辑 "或"，支持两个或更多操作数 |
...
- 一个 `NCLOB`/`NVARCHAR` 列用于文本/内容
- 一个 `NCLOB`/`NVARCHAR` 列用于元数据
- 一个 `REAL_VECTOR` 列用于嵌入向量

允许额外的列；确保它们对新文档插入接受 NULL 值。
...
由于 HANA 的 dbapi 驱动程序默认以 Buffer 对象输出向量列，我们将创建一个辅助函数将其转换为数字列表。
...
- 一个 `NCLOB` 或 `NVARCHAR` 类型的列用于嵌入的文本/内容
- 一个 `NCLOB` 或 `NVARCHAR` 类型的列用于元数据
- 一个 `REAL_VECTOR` 或 `HALF_VECTOR` 类型的列用于嵌入向量

表可以包含额外的列。当新的 Document 插入表中时，这些额外列必须允许 NULL 值。
...
为了允许灵活的元数据值，所有元数据默认以 JSON 格式存储在元数据列中。如果某些使用的元数据键和值类型是已知的，它们可以通过创建以键名为列名的目标表并通过 `specificMetadataColumns` 列表将它们传递给 HanaDB 构造函数来存储在额外列中。与这些值匹配的元数据键会在插入时复制到特殊列中。过滤器会对 `specificMetadataColumns` 列表中的键使用特殊列而不是元数据 JSON 列。
...
特殊列对 LangChain 接口的其余部分完全透明。一切都与以前一样工作，只是性能更好。
...
将加载的文档分块添加到表中。对于本示例，我们删除可能从之前运行中存在的任何以前的内容。
...
使用“欧几里德距离”查询相同的内容。结果应与“余弦相似度”相同。
...
`最大边际相关性` 优化了与查询的相似性以及所选文档之间的多样性。前 20 个（fetch_k）项目将从数据库中检索。然后 MMR 算法将找到最佳的 2 个（k）匹配。
...
- **相似性函数**：索引的相似性函数默认为**余弦相似度**。如果你想使用不同的相似性函数（例如 `L2` 距离），你需要在初始化 `HanaDB` 实例时指定它。
- **默认参数**：在 `createHnswIndex` 函数中，如果用户未提供 `m`、`efConstruction` 或 `efSearch` 等参数的自定义值，将自动使用默认值（例如 `m=64`、`efConstruction=128`、`efSearch=200`）。这些值确保索引以合理的性能创建，无需用户干预。
