...
有关 SAP HANA 向量存储的设置详情，请参见[向量存储：SAP HANA](/oss/integrations/vectorstores/sap_hanavector) 指南。
...
为了能够以良好的性能进行自查询，我们为 HANA 中的向量存储表创建额外的元数据字段：
...
现在是核心部分：以下是如何为 HANA 向量存储构建 SelfQueryRetriever：
