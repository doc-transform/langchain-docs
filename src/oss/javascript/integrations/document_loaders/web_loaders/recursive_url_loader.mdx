...
本笔记提供了开始使用 [RecursiveUrlLoader](/oss/integrations/document_loaders/) 的快速概览。有关 RecursiveUrlLoader 所有功能和配置的详细文档请参览 [API 参考](https://api.js.langchain.com/classes/langchain_community_document_loaders_web_recursive_url.RecursiveUrlLoader.html)。
...
从网站加载内容时，我们可能想处理加载页面上的所有 URL。

例如，让我们看看 LangChain.js 介绍文档。

它有许多有趣的子页面，我们可能想要批量加载、拆分和检索。

挑战在于遍历子页面树并组装列表！

我们使用 `RecursiveUrlLoader` 来完成此操作。

这也给了我们排除某些子页面、自定义提取器等的灵活性。

## 设置

要访问 `RecursiveUrlLoader` 文档加载器，你需要安装 `@langchain/community` 集成和 [`jsdom`](https://www.npmjs.com/package/jsdom) 包。
...
我们还建议添加 [`html-to-text`](https://www.npmjs.com/package/html-to-text) 或
[`@mozilla/readability`](https://www.npmjs.com/package/@mozilla/readability) 等包用于从页面提取原始文本。
...
然而，由于很难执行完美的过滤，你可能仍然会在结果中看到一些不相关的结果。如果需要，你可以自己对返回的文档执行过滤。大多数时候，返回的结果已经足够好。

---

## API 参考

所有 RecursiveUrlLoader 功能和配置的详细文档请参览 [API 参考](https://api.js.langchain.com/classes/langchain_community_document_loaders_web_recursive_url.RecursiveUrlLoader.html)。
