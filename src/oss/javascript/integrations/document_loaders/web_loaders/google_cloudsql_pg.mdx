...
[Cloud SQL](https://cloud.google.com/sql) 是一个完全托管的关系型数据库服务，提供高性能、无缝集成和令人印象深刻的可扩展性，并提供 PostgreSQL 等数据库引擎。

本指南提供了如何使用 Cloud SQL for PostgreSQL 通过 `PostgresLoader` 类加载文档的快速概览。
...
4.  [设置认证。](https://cloud.google.com/docs/authentication)
...
使用 `gcloud auth login` 命令在本地认证你的 Google Cloud 账户。
...
要使用 PostgresLoader 类，你需要安装 `@langchain/google-cloud-sql-pg`
包并按照以下步骤操作。

首先，你需要登录你的 Google Cloud 账户并根据你的 Google Cloud 项目设置以下环境变量；这些将根据你希望如何配置（fromInstance、fromEngine、fromEngineArgs）PostgresEngine 实例来定义：
...
加载器从表中返回文档列表，使用第一列作为 page_content，所有其他列作为 metadata。默认表将第一列作为 page_content，第二列作为 metadata（JSON）。每行成为一个文档。
...
加载器返回文档列表，每行一个文档，页面内容以指定的字符串格式表示，即 text（空格分隔拼接）、JSON、YAML、CSV 等。JSON 和 YAML 格式包含标题，而 text 和 CSV 不包含字段标题。
