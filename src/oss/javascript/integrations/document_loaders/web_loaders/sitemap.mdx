...
description: "使用 LangChain JavaScript 集成 SitemapLoader 文档加载器。"
---

本笔记介绍如何使用 [`SitemapLoader`](https://api.js.langchain.com/classes/_langchain_community.document_loaders_web_sitemap.SitemapLoader.html) 类将站点地图加载为 `Document`。
...
传入的 URL 必须包含 `.xml` 站点地图路径，否则将在 URL 后附加默认的 `/sitemap.xml`。
...
或者，如果你只想加载站点地图而不加载站点地图中每个页面的内容，你可以使用 `parseSitemap` 方法：
