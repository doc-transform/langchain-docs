...
本笔记提供了开始使用 [PuppeteerWebBaseLoader](/oss/integrations/document_loaders/) 的快速概览。有关 PuppeteerWebBaseLoader 所有功能和配置的详细文档请参览 [API 参考](https://api.js.langchain.com/classes/langchain_community_document_loaders_web_puppeteer.PuppeteerWebBaseLoader.html)。

Puppeteer 是一个 Node.js 库，提供了用于控制无头 Chrome 或 Chromium 的高级 API。你可以使用 Puppeteer 自动化网页交互，包括从需要 JavaScript 渲染的动态网页提取数据。
...
要访问 `PuppeteerWebBaseLoader` 文档加载器，你需要安装 `@langchain/community` 集成包和 `puppeteer` peer 依赖。
...
以下是你可以通过 PuppeteerWebBaseLoaderOptions 接口传递给 PuppeteerWebBaseLoader 构造函数的参数说明：
...
1. `launchOptions`：一个可选对象，指定传递给 puppeteer.launch() 方法的额外选项。可以包含 headless 标志以在无头模式下启动浏览器，或 slowMo 选项以减慢 Puppeteer 的操作使其更容易跟踪。

2. `gotoOptions`：一个可选对象，指定传递给 page.goto() 方法的额外选项。可以包含 timeout 选项来指定最大导航时间（毫秒），或 waitUntil 选项来指定何时视为导航成功。

3. `evaluate`：一个可选函数，可用于使用 page.evaluate() 方法在页面上执行 JavaScript 代码。这对于从页面提取数据或与页面元素交互非常有用。该函数应返回一个解析为包含评估结果字符串的 Promise。

通过将这些选项传递给 `PuppeteerWebBaseLoader` 构造函数，你可以自定义加载器的行为，并使用 Puppeteer 的强大功能来爮取和交互网页。

## 截屏

要对网站进行截屏，像上面一样初始化加载器，然后调用 `.screenshot()` 方法。
这将返回一个 `Document` 实例，其中页面内容是 base64 编码的图像，metadata 包含一个 `source` 字段，值为页面的 URL。
...
所有 PuppeteerWebBaseLoader 功能和配置的详细文档请参览 [API 参考](https://api.js.langchain.com/classes/langchain_community_document_loaders_web_puppeteer.PuppeteerWebBaseLoader.html)。
