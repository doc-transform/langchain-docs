...
description: "使用 LangChain JavaScript 集成 LangSmithLoader 文档加载器。"
---


本笔记提供了开始使用 [LangSmithLoader](/oss/integrations/document_loaders/) 的快速概览。有关所有 `LangSmithLoader` 功能和配置的详细文档请参览 [API 参考](https://api.js.langchain.com/classes/_langchain_core.document_loaders_langsmith.LangSmithLoader.html)。
...
要访问 LangSmith 文档加载器，你需要安装 `@langchain/core`，创建一个 [LangSmith](https://langsmith.com/) 账户并获取 API 密钥。

### 凭据

在 [langsmith.com](https://langsmith.com) 注册并生成 API 密钥。完成后设置 `LANGSMITH_API_KEY` 环境变量：
...
所有 `LangSmithLoader` 功能和配置的详细文档请参览 [API 参考](https://api.js.langchain.com/classes/_langchain_core.document_loaders_langsmith.LangSmithLoader.html)
