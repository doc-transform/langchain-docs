...
文档加载器提供了一个**标准接口**，用于将不同来源（如 Slack、Notion 或 Google Drive）的数据读取为 LangChain 的 @[Document] 格式。
这确保了无论数据来源如何，都能以一致的方式处理。
...
- `load()`：一次加载所有文档。
- `loadAndSplit()`：一次加载所有文档并将其拆分为更小的文档。
...
LangChain.js 将文档加载器分为两种类型：

- [文件加载器](/oss/integrations/document_loaders/file_loaders/)，从本地文件系统将数据加载为 LangChain 格式。
- [网络加载器](/oss/integrations/document_loaders/web_loaders/)，从远程源加载数据。
...
| [EPUB](/oss/integrations/document_loaders/file_loaders/epub) | 加载 EPUB 文件，支持可选的章节拆分 | 包 |
...
| [`MultiFileLoader`](/oss/integrations/document_loaders/file_loaders/multi_file) | 从多个单独的文件路径加载数据 | 包 |
...
| [`Puppeteer`](/oss/integrations/document_loaders/web_loaders/web_puppeteer) | 使用 Puppeteer（无头 Chrome）加载动态网页 | ❌ | 包 |
| [`FireCrawl`](/oss/integrations/document_loaders/web_loaders/firecrawl) | 爬取网站并转换为 LLM 可用的 Markdown | ✅ | API |
...
| [`Browserbase`](/oss/integrations/document_loaders/web_loaders/browserbase) | 使用托管的无头浏览器和隐身模式加载网页 | ✅ | API |
...
| [Airtable](/oss/integrations/document_loaders/web_loaders/airtable) | 从 Airtable 数据库加载记录 | ✅ | API |
...
| [Soniox](/oss/integrations/document_loaders/web_loaders/soniox) | 使用 Soniox API 转录多语言音频文件，支持可选翻译 | ✅ | API |
