...
description: "使用 LangChain JavaScript 集成 UnstructuredLoader 文档加载器。"
---
...
本文档提供了开始使用 `UnstructuredLoader` [文档加载器](/oss/integrations/document_loaders)的快速概览。有关所有 `UnstructuredLoader` 功能和配置的详细文档，请前往 [API 参考](https://api.js.langchain.com/classes/langchain_community_document_loaders_fs_unstructured.UnstructuredLoader.html)。
...
要使用 `UnstructuredLoader` 文档加载器，你需要安装 `@langchain/community` 集成包，并创建一个 Unstructured 账户获取 API 密钥。

### 本地运行

你可以使用 Docker 在本地计算机上运行 Unstructured。为此，你需要安装 Docker。你可以在[此处](https://docs.docker.com/get-docker/)找到安装 Docker 的说明。
...
你还可以使用 [`UnstructuredDirectoryLoader`](https://api.js.langchain.com/classes/langchain.document_loaders_fs_unstructured.UnstructuredDirectoryLoader.html) 加载目录中的所有文件，它继承自 [`DirectoryLoader`](/oss/integrations/document_loaders/file_loaders/directory)：
