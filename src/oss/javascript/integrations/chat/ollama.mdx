---
title: "ChatOllama 集成"
description: "使用 LangChain JavaScript 集成 ChatOllama 对话模型。"
---

[Ollama](https://ollama.ai/) 允许你在本地运行开源大语言模型（LLM），如 Llama 3.1。

Ollama 将模型权重、配置和数据打包到一个由 Modelfile 定义的单一包中。它优化了设置和配置细节，包括 GPU 的使用。

本指南将帮助你开始使用 `ChatOllama` [对话模型](/oss/langchain/models)。所有 `ChatOllama` 功能和配置的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_ollama.ChatOllama.html)。

## 概览

### 集成详情

Ollama 允许你使用具有不同能力的各种模型。下表中的某些字段仅适用于 Ollama 提供的部分模型。

支持的模型和模型变体的完整列表，请参见 [Ollama 模型库](https://ollama.com/search)并按标签搜索。
...
Ollama 现已对[部分可用模型](https://ollama.com/search?c=tools)支持原生工具调用。以下示例展示了如何从 Ollama 模型调用工具。
...
Ollama 原生支持所有模型的[结构化输出](https://docs.ollama.com/capabilities/structured-outputs)，允许你通过调用 `.withStructuredOutput()` 强制模型返回特定格式。
...
如果你更喜欢通过工具调用使用结构化输出，请传入 `method: "functionCalling"` 选项：
...
你可以将图片作为消息 `content` 字段的一部分传递给[支持多模态](/oss/langchain/messages#multimodal)的模型，如下所示：
...
所有 ChatOllama 功能和配置的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_ollama.ChatOllama.html)。
