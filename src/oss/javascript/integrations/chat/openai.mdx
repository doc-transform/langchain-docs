...
title: "ChatOpenAI 集成"
sidebarTitle: "Chat"
description: "使用 LangChain JavaScript 集成 ChatOpenAI 对话模型。"
---

[OpenAI](https://en.wikipedia.org/wiki/OpenAI) 是一家人工智能（AI）研究实验室。

本指南将帮助你开始使用 ChatOpenAI [对话模型](/oss/langchain/models)。所有 ChatOpenAI 功能和配置的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_openai.ChatOpenAI.html)。

<Note>
    **Chat Completions API 兼容性**

    `ChatOpenAI` 与 OpenAI 的（旧版）[Chat Completions API](https://platform.openai.com/docs/guides/completions) 完全兼容。如果你想连接到其他支持 Chat Completions API 的模型提供商，也可以这样做——参见[说明](/oss/integrations/chat#chat-completions-api)。
</Note>

<Info>
    **在 Azure 上托管的 OpenAI 模型**
...
要访问 OpenAI 对话模型，你需要创建一个 OpenAI 账户、获取 API 密钥，并安装 `@langchain/openai` 集成包。

### 凭据

前往 [OpenAI 官网](https://platform.openai.com/)注册并生成 API 密钥。完成后设置 `OPENAI_API_KEY` 环境变量：
...
你可以通过传入 `configuration` 参数来自定义 SDK 发送请求的基础 URL：
...
如果你在 Azure OpenAI 上托管，请参见[专门页面](/oss/integrations/chat/azure)。

## 自定义请求头

你可以在相同的 `configuration` 字段中指定自定义请求头：
...
某些代理或第三方提供商提供的 API 接口与 OpenAI 基本相同，但不支持近期新增的 `stream_options` 参数来返回流式用量。你可以通过禁用流式用量来使用 @[`ChatOpenAI`] 访问这些提供商：
...
你可以通过传入相应的 `modelName` 参数来调用微调的 OpenAI 模型。
...
如果你需要 logprobs 或 token 用量等额外信息，这些信息将直接在 `invoke` 响应的消息 `response_metadata` 字段中返回。
...
[自定义工具](https://platform.openai.com/docs/guides/function-calling#custom-tools)支持任意字符串输入的工具。当你预期字符串参数较长或较复杂时，它们尤其有用。

如果你使用支持自定义工具的模型，可以使用 @[`ChatOpenAI`] 类和 `customTool` 函数来创建自定义工具。
...
自 2024 年 8 月 6 日起，OpenAI 支持在调用工具时使用 `strict` 参数，该参数将强制模型遵守工具参数 Schema。[查看更多](https://platform.openai.com/docs/guides/function-calling)。
...
我们也可以将 `strict: true` 传递给 [`.withStructuredOutput()`](https://js.langchain.com/docs/how_to/structured_output/#the-.withstructuredoutput-method)。示例如下：
...
OpenAI 支持 [Responses](https://platform.openai.com/docs/guides/responses-vs-chat-completions) API，它面向构建 [Agent 应用](/oss/langchain/agents)。它包括一套[内置工具](https://platform.openai.com/docs/guides/tools?api-mode=responses)，包括网络和文件搜索。它还支持[会话状态](https://platform.openai.com/docs/guides/conversation-state?api-mode=responses)管理，允许你无需显式传入之前的消息即可继续对话线程。

当使用这些功能时，`ChatOpenAI` 会自动路由到 Responses API。你也可以在实例化 `ChatOpenAI` 时指定 `useResponsesApi: true`。

### 内置工具

为 @[`ChatOpenAI`] 配备内置工具可以让其响应基于外部信息，例如文件或网络中的上下文。模型生成的 [AIMessage](/oss/langchain/messages/#aimessage) 将包含内置工具调用的信息。

#### 网络搜索

要触发网络搜索，像传入其他工具一样将 `{"type": "web_search_preview"}` 传递给模型。
...
请注意，响应包含结构化的[内容块](/oss/langchain/messages/#message-content)，包括响应文本和 OpenAI [注解](https://platform.openai.com/docs/guides/tools-web-search?api-mode=responses#output-and-citations)引用其来源。输出消息还将包含任何工具调用的信息。

#### 文件搜索

要触发文件搜索，像传入其他工具一样将[文件搜索工具](https://platform.openai.com/docs/guides/tools-file-search)传递给模型。你需要填充一个 OpenAI 管理的向量存储并在工具定义中包含向量存储 ID。详情请参见 [OpenAI 文档](https://platform.openai.com/docs/guides/tools-file-search)。
...
与[网络搜索](#web-search)一样，响应将包含带有引用的内容块。它还将包含内置工具调用的信息。

#### 计算机使用

ChatOpenAI 支持 `computer-use-preview` 模型，这是一个专门用于内置计算机使用工具的专用模型。要启用，像传入其他工具一样传入[计算机使用工具](https://platform.openai.com/docs/guides/tools-computer-use)。

目前计算机使用的工具输出位于 `AIMessage.additional_kwargs.tool_outputs` 中。要回复计算机使用工具调用，你需要在创建相应的 `ToolMessage` 时设置 `additional_kwargs.type: "computer_call_output"`。

详情请参见 [OpenAI 文档](https://platform.openai.com/docs/guides/tools-computer-use)。
...
ChatOpenAI 允许你使用内置的[代码解释器工具](https://platform.openai.com/docs/guides/tools-code-interpreter)来支持沙箱中的代码生成和执行。
...
请注意，上述命令创建了一个新的[容器](https://platform.openai.com/docs/guides/tools-code-interpreter#containers)。我们可以通过指定现有的容器 ID 来在调用之间重用容器。
...
ChatOpenAI 支持内置的[远程 MCP 工具](https://platform.openai.com/docs/guides/tools-remote-mcp)，允许在 OpenAI 服务器上发生模型生成的 MCP 服务器调用。
...
当指示时，OpenAI 将请求批准后再对远程 MCP 服务器进行调用。

在上述命令中，我们指示模型从不请求批准。我们也可以配置模型始终请求批准，或者始终请求特定工具的批准：
...
使用此配置，响应可以包含类型为 `mcp_approval_request` 的工具输出。要提交批准请求的批准，你可以将结构化内容块放入后续消息中：
...
ChatOpenAI 允许你使用内置的[图像生成工具](https://platform.openai.com/docs/guides/tools-image-generation)通过响应 API 在多轮对话中创建图像。
...
当使用原因模型（如 `o1`）时，`withStructuredOutput` 的默认方法是 OpenAI 的内置方法（相当于将 `method: "jsonSchema"` 作为选项传递给 `withStructuredOutput`）。JSON 模式在其他模型中大多工作正常，但有一个重要的注意事项：在定义模式时，`z.optional()` 不被尊重，你应该使用 `z.nullable()`。
...
较新的 OpenAI 模型会在输入超过一定大小（撰写本文时为 1024 个 token）时自动[缓存部分提示词](https://openai.com/index/api-prompt-caching/)，以降低需要长上下文场景的成本。
...
部分 OpenAI 模型（如 `gpt-4o` 和 `gpt-4o-mini` 系列）支持[预测输出](https://platform.openai.com/docs/guides/latency-optimization#use-predicted-outputs)，允许你提前传入 LLM 预期输出的已知部分以减少延迟。这对于编辑文本或代码等只有模型输出的一小部分会改变的场景非常有用。
...
请注意，目前预测输出会作为额外 token 计费，将增加你的用量和成本，以换取降低的延迟。

## 音频输出

部分 OpenAI 模型（如 `gpt-4o-audio-preview`）支持生成音频输出。此示例展示了如何使用该功能：
...
我们可以看到音频数据在 `data` 字段中返回。我们还获得了一个 `expires_at` 日期字段。该字段表示音频响应在服务器上不再可用于多轮对话的日期。

### 流式音频输出

OpenAI 还支持流式音频输出。示例如下：
...
这些模型还支持将音频作为输入传递。为此，你必须如下所示指定 `input_audio` 字段：
...
所有 ChatOpenAI 功能和配置的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_openai.ChatOpenAI.html)。
