---
title: "Ollama 函数调用集成"
description: "使用 LangChain JavaScript 集成 Ollama 函数调用对话模型。"
---
...
LangChain 提供了一个实验性封装器，用于通过 [Ollama](https://github.com/jmorganca/ollama) 本地运行的开源模型，
为其提供与 OpenAI Functions 相同的 API。

请注意，更强大的模型在处理复杂 Schema 和/或多个函数时会表现得更好。以下示例
使用的是 [Mistral](https://ollama.ai/library/mistral)。

<Warning>
**这是一个实验性封装器，尝试为不原生支持工具调用的模型添加工具调用支持。请谨慎使用。**
...
你可以像初始化标准 `ChatOllama` 实例一样初始化这个封装器：
...
<Tip>
你可以在[此处](https://smith.langchain.com/public/74692bfc-0224-4221-b187-ddbf20d7ecc0/r)查看一个简单的 LangSmith 追踪示例
</Tip>

## 自定义

在底层，这使用 Ollama 的 JSON 模式来约束输出为 JSON，然后将工具 Schema 作为 JSON Schema 传入提示词。

由于不同的模型有不同的优势，传入你自己的系统提示词可能会有所帮助。以下是一个示例：
