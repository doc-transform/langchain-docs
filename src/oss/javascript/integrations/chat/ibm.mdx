...
description: "使用 LangChain JavaScript 集成 IBM watsonx.ai 对话模型。"
---

本文将帮助你开始使用 IBM watsonx.ai [对话模型](/oss/langchain/models)。所有 `IBM watsonx.ai` 功能和配置的详细文档请参见 [IBM watsonx.ai](https://api.js.langchain.com/modules/_langchain_community.chat_models_ibm.html)。
...
要访问 IBM watsonx.ai 模型，你需要创建一个 IBM watsonx.ai 账户、获取 API 密钥，并安装 `@langchain/community` 集成包。

### 凭据

前往 [IBM Cloud](https://cloud.ibm.com/login) 注册 IBM watsonx.ai 并生成 API 密钥，或提供以下展示的其他身份验证形式。
...
- 你必须提供 `spaceId`、`projectId` 或 `idOrName`（部署 ID），除非你使用轻量级引擎（无需指定任何一个，请参阅 [watsonx.ai 文档](https://www.ibm.com/docs/en/cloud-paks/cp-data/5.0.x?topic=install-choosing-installation-mode)）
- 根据你预配置服务实例的区域，使用正确的 serviceUrl。
...
要使用模型网关与 Langchain 配合，你需要预先通过 `@ibm-cloud/watsonx-ai` SDK 或 `watsonx.ai` API 创建提供商并添加模型。请参考以下文档：
