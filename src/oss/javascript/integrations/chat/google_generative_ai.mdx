...
description: "使用 LangChain JavaScript 集成 ChatGoogleGenerativeAI 对话模型。"
---

[Google AI](https://ai.google.dev/) 提供多种不同的对话模型，包括强大的 Gemini 系列。有关最新模型、其功能、上下文窗口等信息，请参见 [Google AI 文档](https://ai.google.dev/gemini-api/docs/models/gemini)。

本文将帮助你开始使用 `ChatGoogleGenerativeAI` [对话模型](/oss/langchain/models)。所有 `ChatGoogleGenerativeAI` 功能和配置的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_google_genai.ChatGoogleGenerativeAI.html)。
...
你可以通过 `@langchain/google-genai` 集成包中的 `ChatGoogleGenerativeAI` 类来访问 Google 的 `gemini` 和 `gemini-vision` 模型，以及其他生成式模型。


<Tip>
你也可以通过 LangChain VertexAI 和 VertexAI-web 集成访问 Google 的 `gemini` 系列模型。点击[此处](/oss/integrations/chat/google_vertex_ai)查看文档。
</Tip>
