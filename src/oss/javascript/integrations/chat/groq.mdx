---
title: "ChatGroq 集成"
description: "使用 LangChain JavaScript 集成 ChatGroq 对话模型。"
---

[Groq](https://groq.com/) 是一家提供快速 AI 推理的公司，由 LPU™ AI 推理技术驱动，提供快速、经济实惠且节能的 AI 服务。

本文将帮助你开始使用 ChatGroq [对话模型](/oss/langchain/models)。所有 ChatGroq 功能和配置的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_groq.ChatGroq.html)。
...
要访问 ChatGroq 模型，你需要创建一个 Groq 账户、获取 API 密钥，并安装 `@langchain/groq` 集成包。

### 凭据

要使用 Groq API，你需要一个 API 密钥。你可以在[此处](https://wow.groq.com/)注册 Groq 账户并创建 API 密钥。
然后，你可以在终端中将 API 密钥设置为环境变量：
...
所有 ChatGroq 功能和配置的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_groq.ChatGroq.html)。
