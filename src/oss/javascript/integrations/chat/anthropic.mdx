...
sidebarTitle: "Chat"
description: "使用 LangChain JavaScript 集成 ChatAnthropic 对话模型。"
---

[Anthropic](https://www.anthropic.com/) 是一家 AI 安全与研究公司，是 Claude 的创建者。

本文将帮助你开始使用 Anthropic [对话模型](/oss/langchain/models)。所有 `ChatAnthropic` 功能和配置的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_anthropic.ChatAnthropic.html)。
...
你需要注册并获取一个 [Anthropic API 密钥](https://www.anthropic.com/)，并安装 `@langchain/anthropic` 集成包。

### 凭据

前往 [Anthropic 官网](https://www.anthropic.com/)注册并生成 API 密钥。完成后设置 `ANTHROPIC_API_KEY` 环境变量：
...
Anthropic 模型与大多数其他模型之间的一个关键区别是，单个 Anthropic @[`AIMessage`] 的内容可以是单个字符串，也可以是**内容块列表**。例如，当 Anthropic 模型[调用工具](/oss/langchain/tools)时，工具调用是消息内容的一部分（同时也通过标准化的 `AIMessage.tool_calls` 字段暴露）：
...
Anthropic 支持[缓存部分提示词](https://platform.claude.com/docs/en/build-with-claude/prompt-caching)，以降低需要长上下文场景的成本。你可以缓存工具以及完整消息和单个内容块。

包含一个或多个带有 `"cache_control": { "type": "ephemeral" }` 字段的内容块或工具定义的初始请求会自动缓存该部分提示词。这个初始缓存步骤会产生额外费用，但后续请求将按降低的费率计费。缓存的有效期为 5 分钟，但每次命中缓存时都会刷新。

目前还有一个最小可缓存提示词长度限制，因模型而异。你可以在[此处](https://platform.claude.com/docs/en/build-with-claude/prompt-caching#structuring-your-prompt)查看此信息。

当前需要使用 beta 请求头来初始化模型。以下是缓存包含 LangChain [概念文档](/oss/concepts/)的系统消息部分的示例：
...
我们可以看到原始用量字段中新增了 `cache_creation_input_tokens` 字段。
...
你也可以通过在工具定义中设置相同的 `"cache_control": { "type": "ephemeral" }` 来缓存工具。这目前需要你以 [Anthropic 原始工具格式](https://platform.claude.com/docs/en/agents-and-tools/tool-use/overview)绑定工具。示例如下：
...
有关提示词缓存工作原理的更多信息，请参见 [Anthropic 文档](https://platform.claude.com/docs/en/build-with-claude/prompt-caching#how-prompt-caching-works)。

## 自定义客户端

Anthropic 模型[可能托管在 Google Vertex 等云服务上](https://platform.claude.com/docs/en/build-with-claude/claude-on-vertex-ai)，这些服务依赖与主 Anthropic 客户端具有相同接口的不同底层客户端。你可以通过提供一个 `createClient` 方法来访问这些服务，该方法返回一个已初始化的 Anthropic 客户端实例。示例如下：
...
Anthropic 支持[引用](https://platform.claude.com/docs/en/build-with-claude/citations)功能，允许 Claude 根据用户提供的源材料将上下文附加到其回答中。这些源材料可以以[文档内容块](https://platform.claude.com/docs/en/build-with-claude/citations#document-types)（描述完整文档）或[搜索结果](https://platform.claude.com/docs/en/build-with-claude/search-results)（描述从检索系统返回的相关段落或片段）的形式提供。当查询中包含 `"citations": { "enabled": true }` 时，Claude 可能会在其响应中生成对提供材料的直接引用。

### 文档示例

在此示例中，我们传入一个[纯文本文档](https://platform.claude.com/docs/en/build-with-claude/citations#plain-text-documents)。在后台，Claude 会[自动将](https://platform.claude.com/docs/en/build-with-claude/citations#plain-text-documents)输入文本分割成句子，用于生成引用。
...
在此示例中，我们将[搜索结果](https://platform.claude.com/docs/en/build-with-claude/search-results)作为消息内容的一部分传入。这允许 Claude 在其响应中引用你自己检索系统中的特定段落或片段。

当你希望 Claude 引用特定知识集中的信息，但想要直接提供自己预取/缓存的内容而不是让模型自动搜索或检索时，这种方法尤其有用。
...
你也可以使用工具提供搜索结果，让模型可以在其响应中引用。这非常适合 RAG（或[检索增强生成](https://en.wikipedia.org/wiki/Retrieval-augmented_generation)）工作流，其中 Claude 可以决定何时何地检索信息。当将这些信息以[搜索结果](https://platform.claude.com/docs/en/build-with-claude/search-results)的形式返回时，它赋予了 Claude 从工具返回的材料中创建引用的能力。

以下是如何创建一个工具，以 Anthropic 引用 API 所期望的格式返回搜索结果：
...
在[此处](https://js.langchain.com/docs/concepts/rag/)了解更多关于 RAG 在 LangChain 中如何工作的信息

[了解更多关于工具调用的信息](/oss/langchain/tools)

### 与文本分割器配合使用

Anthropic 还允许你使用[自定义文档](https://platform.claude.com/docs/en/build-with-claude/citations#custom-content-documents)类型来指定自己的分割。LangChain 文本分割器可用于生成有意义的分割。请看下面的示例，我们将 LangChain.js README（一个 Markdown 文档）进行分割并作为上下文传递给 Claude：
...
Anthropic 支持上下文编辑功能，可以自动管理模型的上下文窗口（例如通过清除工具结果）。

有关详情和配置选项，请参见 [Anthropic 文档](https://platform.claude.com/docs/en/build-with-claude/context-editing)。
...
所有 ChatAnthropic 功能和配置的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_anthropic.ChatAnthropic.html)。
