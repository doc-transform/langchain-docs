...
description: "使用 LangChain JavaScript 集成 ChatMiniMax 对话模型。"
---

[Minimax](https://api.minimax.chat) 是一家中国初创公司，为企业和个人提供自然语言处理模型。

本示例展示了使用 LangChain.js 与 Minimax 进行交互。
...
<Tip>
我们正在统一所有包的模型参数。现在建议使用 `model` 而不是 `modelName`，使用 `apiKey` 作为 API 密钥。
</Tip>
