---
title: "ChatCohere 集成"
description: "使用 LangChain JavaScript 集成 ChatCohere 对话模型。"
---

[Cohere](https://cohere.com/) 是一家加拿大初创公司，提供自然语言处理模型，帮助企业改善人机交互。

本文将帮助你开始使用 Cohere [对话模型](/oss/langchain/models)。所有 `ChatCohere` 功能和配置的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_cohere.ChatCohere.html)。
...
要使用 LangChain.js Cohere 集成，你需要一个 API 密钥。
你可以在[此处](https://dashboard.cohere.com/welcome/register)注册 Cohere 账户并创建 API 密钥。

你需要先安装 [`@langchain/cohere`](https://www.npmjs.com/package/@langchain/cohere) 包。

### 凭据

前往 [Cohere 官网](https://dashboard.cohere.com/welcome/register)注册并生成 API 密钥。完成后设置 `COHERE_API_KEY` 环境变量：
...
Cohere 原生支持 RAG。
你可以将文档作为上下文传递给 API 请求，Cohere 的模型会在生成响应时使用它们。
...
API 还允许使用非静态文档的其他连接。
其中一个例子是 `web-search` 连接器，它允许你传入查询，API 会搜索网络上的相关文档。
以下示例展示了如何使用该功能。
...
所有 ChatCohere 功能和配置的详细文档请参见 [API 参考](https://api.js.langchain.com/classes/langchain_cohere.ChatCohere.html)。
