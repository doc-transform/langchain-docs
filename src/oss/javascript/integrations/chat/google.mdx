...
title: "ChatGoogle 集成"
sidebarTitle: "Chat"
description: "使用 LangChain JavaScript 集成 ChatGoogle 对话模型。"
---

此库支持访问 Google 的多种模型，包括 Gemini
系列模型和 Nano Banana 图像生成模型。你可以通过 Google 的 [Google AI](https://ai.google.dev/) API（有时也
称为 Generative AI API 或 AI Studio API）或通过 Google Cloud Platform
[Vertex AI](https://cloud.google.com/vertex-ai) 服务来访问这些模型。

本文将帮助你开始使用 `ChatGoogle` [对话模型](/oss/langchain/models)。
所有 `ChatGoogle` 功能和配置的详细文档请参见
[API 参考](https://api.js.langchain.com/classes/langchain_google.ChatGoogle.html)。

<Tip>
  `@langchain/google` 是所有新 Google Gemini 集成的推荐包。
  它替代了旧的 [`@langchain/google-genai`](/oss/integrations/chat/google_generative_ai)
  和 [`@langchain/google-vertexai`](/oss/integrations/chat/google_vertex_ai) 包。
  有关迁移详情请参见[旧版包](/oss/integrations/providers/google#legacy-packages)。
</Tip>
...
要通过 Google AI Studio（有时称为 Generative AI API）使用模型，你需要一个 API 密钥。你可以从
[Google AI Studio](https://aistudio.google.com/app/apikey) 获取。

获取 API 密钥后，可以将其设置为环境变量：
