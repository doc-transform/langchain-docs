...
在本教程中，你将学习如何使用 LangChain [Agent](/oss/langchain/agents) 构建一个能够回答 SQL 数据库相关问题的 Agent。

从宏观上看，该 Agent 将：

<Steps>
<Step title="从数据库获取可用的表和架构" />
<Step title="判断哪些表与问题相关" />
<Step title="获取相关表的架构信息" />
<Step title="根据问题和架构信息生成查询语句" />
<Step title="使用 LLM 检查查询中的常见错误" />
<Step title="执行查询并返回结果" />
<Step title="修正数据库引擎报告的错误，直到查询成功" />
<Step title="根据结果形成回答" />
</Steps>

<Warning>
构建 SQL 数据库问答系统需要执行模型生成的 SQL 查询。这样做存在固有风险。请确保数据库连接权限始终尽可能缩小到 Agent 需要的最小范围。这将降低（但不能完全消除）构建模型驱动系统的风险。
</Warning>
...
- LangChain [Agent](/oss/langchain/agents)
- [人工审核](/oss/langchain/human-in-the-loop)流程
...
配置 [LangSmith](https://smith.langchain.com) 以检查链或 Agent 内部的运行情况。然后设置以下环境变量：
...
本教程将创建一个 [SQLite 数据库](https://www.sqlitetutorial.net/sqlite-sample-database/)。SQLite 是一个轻量级数据库，易于配置和使用。我们将加载 `chinook` 数据库，这是一个代表数字媒体商店的示例数据库。
...
:::
:::python
我们将使用 `langchain_community` 包中提供的便捷 SQL 数据库封装器来与数据库交互。该封装器提供了执行 SQL 查询和获取结果的简单接口：
...
## 3. 添加数据库交互工具
:::python
使用 `langchain_community` 包中的 `SQLDatabase` 封装器与数据库交互。该封装器提供了执行 SQL 查询和获取结果的简单接口：
...
使用 `langchain/sql_db` 中的 `SqlDatabase` 封装器与数据库交互。该封装器提供了执行 SQL 查询和获取结果的简单接口：
```typescript
...
使用 @[`create_agent`] 以最少的代码构建一个 [ReAct Agent](https://arxiv.org/pdf/2210.03629)。Agent 会解释请求并生成 SQL 命令，由工具执行。如果命令出错，错误信息会返回给模型。模型可以检查原始请求和新的错误信息，然后生成新的命令。这个过程会持续直到 LLM 成功生成命令或达到最大重试次数。这种向模型提供反馈的模式——本例中是错误信息——非常强大。

使用描述性的系统提示词初始化 Agent 以自定义其行为：
...
Agent 正确地编写了查询、检查了查询并执行它来形成最终响应。

<Note>
    你可以在 [LangSmith trace](https://smith.langchain.com/public/cd2ce887-388a-4bb1-a29d-48208ce50d15/r) 中检查上述运行的所有方面，包括执行的步骤、调用的工具、LLM 看到的提示词等。
</Note>

### （可选）使用 Studio

[Studio](/langsmith/studio) 提供了“客户端”循环和记忆功能，你可以将其作为聊天界面来查询数据库。你可以提问“告诉我数据库的架构”或“展示前 5 个客户的发票”等问题。你将看到生成的 SQL 命令和结果输出。启动详情见下方。
<Accordion title="在 Studio 中运行 Agent">
...
在运行目录中，你需要一个包含以下内容的 `langgraph.json` 文件：
...
在执行之前检查 Agent 的 SQL 查询是否存在意外操作或效率低下的情况，是一个明智的做法。

LangChain Agent 支持内置的[人工审核中间件](/oss/langchain/human-in-the-loop)，可为 Agent 的工具调用添加监督。让我们配置 Agent 在调用 `sql_db_query` 工具时暂停等待人工审核：
...
```
<Note>
我们为 Agent 添加了 [checkpointer](/oss/langchain/short-term-memory) 以允许执行暂停和恢复。有关详情以及可用的中间件配置，请参见[人工审核指南](/oss/langchain/human-in-the-loop)。
</Note>

运行 Agent 时，它现在会在执行 `sql_db_query` 工具之前暂停等待审核：
```python
question = "Which genre on average has the longest tracks?"
...
```
我们可以使用 [Command](/oss/langgraph/use-graph-api#combine-control-flow-and-state-updates-with-command) 恢复执行，在本例中是批准查询：
```python
...
有关详情，请参见[人工审核指南](/oss/langchain/human-in-the-loop)。

:::js
## 4. 执行 SQL 查询

在运行命令之前，先用 `_safe_sql` 检查 LLM 生成的命令：
...
使用 `createAgent` 以最少的代码构建一个 [ReAct Agent](https://arxiv.org/pdf/2210.03629)。Agent 会解释请求并生成 SQL 命令。工具会检查命令的安全性，然后尝试执行。如果命令出错，错误信息会返回给模型。模型可以检查原始请求和新的错误信息，然后生成新的命令。这个过程会持续直到 LLM 成功生成命令或达到最大重试次数。这种向模型提供反馈的模式——本例中是错误信息——非常强大。
...
<Note>
    你可以在 [LangSmith trace](https://smith.langchain.com/public/653d218b-af67-4854-95ca-6abecb9b2520/r) 中检查上述运行的所有方面，包括执行的步骤、调用的工具、LLM 看到的提示词等。
</Note>
...
如需更深入的自定义，请查看[本教程](/oss/langgraph/sql-agent)，了解如何直接使用 LangGraph 原语实现 SQL Agent。
