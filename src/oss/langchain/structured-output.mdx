...
结构化输出允许 Agent 以特定的、可预测的格式返回数据。你无需解析自然语言响应，而是直接获得 JSON 对象、[Pydantic 模型](https://docs.pydantic.dev/latest/concepts/models/#basic-model-usage)或 dataclass 形式的结构化数据，供应用程序直接使用。

<Tip>
本页介绍使用 `create_agent` 创建的 Agent 的结构化输出。如需直接在模型上使用结构化输出（不通过 Agent），请参阅[模型 - 结构化输出](/oss/langchain/models#structured-output)。
</Tip>

LangChain 的 @[`create_agent`] 自动处理结构化输出。用户设定所需的结构化输出 schema，当模型生成结构化数据时，数据会被捕获、验证并返回到 Agent state 的 `'structured_response'` 键中。
...
使用 `response_format` 控制 Agent 返回结构化数据的方式：

- **`ToolStrategy[StructuredResponseT]`**：通过工具调用实现结构化输出
- **`ProviderStrategy[StructuredResponseT]`**：使用提供商原生的结构化输出能力
- **`type[StructuredResponseT]`**：Schema 类型——根据模型能力自动选择最佳策略
- **`None`**：不显式请求结构化输出

当直接传入 schema 类型时，LangChain 自动选择：

- 如果所选模型和提供商支持原生结构化输出（如 [OpenAI](/oss/integrations/providers/openai)、[Anthropic (Claude)](/oss/integrations/providers/anthropic) 或 [xAI (Grok)](/oss/integrations/providers/xai)），则使用 `ProviderStrategy`。
- 对于其他所有模型，使用 `ToolStrategy`。

<Note>
    如果使用 `langchain>=1.1`，对原生结构化输出的支持信息会从模型的 [profile 数据](/oss/langchain/models#model-profiles)中动态读取。如果数据不可用，请使用其他条件或手动指定：
    ```python
    custom_profile = {
...
:::
:::js
结构化输出允许 Agent 以特定的、可预测的格式返回数据。你无需解析自然语言响应，而是直接获得类型化的结构化数据。

<Tip>
本页介绍使用 `createAgent` 创建的 Agent 的结构化输出。如需直接在模型上使用结构化输出（不通过 Agent），请参阅[模型 - 结构化输出](/oss/langchain/models#structured-output)。
</Tip>

LangChain 预构建的 ReAct Agent `createAgent` 自动处理结构化输出。用户设定所需的结构化输出 schema，当模型生成结构化数据时，数据会被捕获、验证并返回到 Agent state 的 `structuredResponse` 键中。
...
控制 Agent 返回结构化数据的方式。你可以提供 Zod 对象或 JSON schema。默认情况下，Agent 使用工具调用策略——通过额外的工具调用来创建输出。某些模型支持原生结构化输出，此时 Agent 会改用该策略。
...
部分模型提供商通过其 API 原生支持结构化输出（如 OpenAI、xAI (Grok)、Gemini、Anthropic (Claude)）。这是可用时最可靠的方式。
...
<ParamField path="schema" required>
    ...
    - **Pydantic 模型**：带字段验证的 `BaseModel` 子类。返回经验证的 Pydantic 实例。
    - **Dataclass**：带类型注解的 Python dataclass。返回字典。
    - **TypedDict**：类型化字典类。返回字典。
    - **JSON Schema**：JSON schema 规范的字典。返回字典。
</ParamField>

<ParamField path="strict">
    可选的布尔参数，用于启用严格 schema 遵循。部分提供商支持（如 [OpenAI](/oss/integrations/chat/openai) 和 [xAI](/oss/integrations/chat/xai)）。默认为 `None`（禁用）。
</ParamField>

当你直接将 schema 类型传递给 @[`create_agent.response_format`][create_agent(response_format)] 且模型支持原生结构化输出时，LangChain 自动使用 `ProviderStrategy`：
...
当你直接将 schema 类型传递给 `createAgent.responseFormat` 且模型支持原生结构化输出时，LangChain 自动使用 `ProviderStrategy`：
...
提供商原生结构化输出具有高可靠性和严格验证，因为模型提供商会强制执行 schema。建议在可用时优先使用。

:::python
<Note>
    如果提供商原生支持所选模型的结构化输出，写 `response_format=ProductReview` 和写 `response_format=ProviderStrategy(ProductReview)` 在功能上等价。

    无论哪种情况，如果不支持结构化输出，Agent 都会回退到工具调用策略。
</Note>
:::
:::js
<Note>
    如果提供商原生支持所选模型的结构化输出，写 `responseFormat: contactInfoSchema` 和写 `responseFormat: providerStrategy(contactInfoSchema)` 在功能上等价。
...
对于不支持原生结构化输出的模型，LangChain 使用工具调用来实现相同效果。这适用于所有支持工具调用的模型（大多数现代模型）。
...
不使用 `tool_message_content` 时，最终的 @[`ToolMessage`] 将是：
```
================================= Tool Message =================================
Name: MeetingAction
...
当模型错误地调用了多个结构化输出工具时，Agent 会在 @[`ToolMessage`] 中提供错误反馈并提示模型重试：
...
当结构化输出不匹配预期 schema 时，Agent 会提供具体的错误反馈：
...
如果 `handle_errors` 是异常类型，Agent 仅在抛出的异常是指定类型时才重试（使用默认错误消息）。其他情况下异常将被抛出。
...
如果 `handle_errors` 是异常元组，Agent 仅在抛出的异常是指定类型之一时才重试（使用默认错误消息）。其他情况下异常将被抛出。
...
for msg in result['messages']:
    # 如果消息是 ToolMessage 对象（不是字典），检查其类名
