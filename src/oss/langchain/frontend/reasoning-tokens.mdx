---
title: 推理 Token
description: 在可折叠块中展示模型的思考和推理过程
---

推理 token 暴露了高级模型（如 OpenAI 的 o1/o3 和 Anthropic 的 Claude 扩展思考）的内部思维过程。这些模型产生结构化的内容块，将推理与最终答案分离，让你可以构建展示模型*如何*得出响应的 UI。
...
2. **文本块**：呈现给用户的最终、精美的响应

这些作为类型化的内容块在 `AIMessage` 中传递，通过 `contentBlocks` 属性访问：
...
<Note>
并非所有模型都产生推理 token。此模式专门适用于支持扩展思考或思维链输出的模型。标准聊天模型只返回文本块。
</Note>

## 使用场景

- **透明度**：展示模型的推理过程以建立用户对答案的信任
- **调试**：检查模型的思维过程以识别哪里出了问题
- **教育工具**：通过揭示 AI 如何解决问题来教学生问题解决方法
- **决策支持**：让领域专家验证推荐背后的推理
- **质量保证**：在受监管行业中审计推理链以确保合规

## 提取推理和文本块

`AIMessage` 上的 `contentBlocks` 数组按生成顺序包含所有块。按 `type` 过滤以分离推理和文本：
...
单条消息可能包含多个推理块（例如，模型暂停推理、产生部分文本，然后继续推理）。将它们连接起来可以获得完整的思维过程。
...
`ThinkingBubble` 将推理 token 展示在一个视觉上独特的、可折叠的容器中。用户可以展开查看完整的思维过程，或折叠以专注于最终答案。
...
当模型仍在生成推理 token 时，展示动画指示器以传达思考正在进行：
...
<Tip>
在流式传输期间，默认保持 ThinkingBubble 折叠，只显示加载动画。在流式传输过程中展开可能会因新 token 到达而导致布局抖动。让用户在推理阶段完成后再展开。
</Tip>

## 渲染完整的 AI 响应

将 `ThinkingBubble` 和标准文本气泡组合为单个 `AIResponse` 组件：
...
并非每条 AI 消息都包含推理块。当 `contentBlocks` 只有文本块时，渲染标准消息气泡，不显示 ThinkingBubble。

### 空推理块

某些模型会产生空推理块作为占位符。过滤掉它们：
...
单条消息可以在推理和文本块之间交替。如果需要保留这种交错，按顺序遍历 `contentBlocks` 而不是按类型分组：
...
- **动画过渡**：平滑的展开/折叠动画提升感知质量
- **考虑无障碍**：在切换按钮上使用适当的 ARIA 属性（`aria-expanded`、`aria-controls`）
