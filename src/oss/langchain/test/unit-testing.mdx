---
title: 单元测试
description: 使用 Mock 聊天模型和内存持久化测试 Agent 逻辑，无需 API 调用。
---

单元测试隔离测试 Agent 的小型确定性组件。通过用内存假对象（即 fixture）替代真实 LLM，你可以编排精确的响应（文本、工具调用和错误），使测试快速、免费且可重复，无需 API 密钥。

:::python

## Mock 聊天模型

LangChain 提供 @[`GenericFakeChatModel`] 用于模拟文本响应。它接受一个响应迭代器（@[`AIMessage`] 对象或字符串），每次调用返回一个。它支持常规和流式使用。
...
要在测试中启用持久化，你可以使用 @[`InMemorySaver`] 检查点器。这允许你模拟多轮对话来测试状态相关的行为：
...
## 使用 `fakeModel` Mock 聊天模型

@[`fakeModel`] 是一个构建器风格的假聊天模型，让你可以编排精确的响应（文本、工具调用、错误）并断言模型接收到的内容。它继承自 @[`BaseChatModel`]，因此可以在任何需要真实模型的地方使用。
...
创建模型，使用 `.respond()` 排队响应，然后调用。每次 `invoke()` 按顺序消费下一个排队的响应：
...
如果模型被调用的次数超过排队响应的数量，它会抛出一个描述性错误：
...
`.respond()` 支持通过传递带有 `tool_calls` 的 @[`AIMessage`] 来实现工具调用：
...
`.respondWithTools()` 是同样功能的简写。不需要构造完整的 `AIMessage`，只需提供工具名称和参数：
...
<Tip>
    `.respond()` 和 `.respondWithTools()` 可以以任意顺序自由混合使用。这在测试 Agent 循环时特别有用，因为模型会交替进行工具调用和文本响应。
</Tip>
...
将 `Error` 传递给 `.respond()` 会使模型在该特定调用中抛出异常。错误可以出现在序列的任意位置：
...
`.alwaysThrow()` 使每次调用都抛出异常，不管队列中有什么。这对测试错误处理和重试逻辑很有用：
...
`.respond()` 也接受一个函数，根据输入消息计算响应。函数接收完整的消息数组，返回一个 @[`BaseMessage`] 或 `Error`：
...
<Note>
    每个函数是一个单独的队列条目，只消费一次。要对多轮复用相同的动态逻辑，需排队多个 `respond` 函数调用。
</Note>
...
传递给 `.withStructuredOutput()` 的 schema 会被忽略。模型始终返回通过 `.structuredResponse()` 配置的值。这使测试专注于应用逻辑而非解析。
...
Agent 框架（如 LangChain Agent 和 LangGraph）会在内部调用 `model.bindTools(tools)`。`fakeModel` 会自动处理。绑定后的模型与原始模型共享相同的响应队列和调用记录，因此不需要特殊设置：
...
<Accordion title="完整示例：使用 vitest 测试一个工具调用 Agent">
...
了解如何使用真实模型提供商 API 测试你的 Agent，请参阅[集成测试](/oss/langchain/test/integration-testing)。
